CREATE OR REPLACE PACKAGE "PKG_PAYMENT4DEBUG" IS
...
END;
/
CREATE OR REPLACE PACKAGE BODY "PKG_PAYMENT4DEBUG" IS
  ...
  (
   ...
   ,p_end_per_date   DATE DEFAULT NULL
    ...
    v_pl_delta NUMBER;
  BEGIN
    xx_pkg_log.save_log('calc_fee_delta - Enter, p_pol_id = ' || p_pol_id || ' ,p_due_date = ' ||
                        p_due_date || ' ,p_adm_cost = ' || p_adm_cost || ' ,p_is_period = ' ||
                        p_is_period || ' ,p_start_per_date = ' || p_start_per_date ||
                        ' ,p_end_per_date = ' || p_end_per_date);
    p_return_sum := 0;
    p_paym_sum   := 0;
    ...
    LOOP
      ...
      END IF;
    END LOOP;
    xx_pkg_log.save_log('calc_fee_delta - End, p_return_sum = ' || p_return_sum || ' ,p_paym_sum = ' ||
                        p_paym_sum);
  END;
  ...
  (
   ...
   ,p_end_per_date   DATE DEFAULT NULL
    ...
    v_prorata   NUMBER;
  BEGIN
    xx_pkg_log.save_log('calc_prodline_fee_delta - Enter, p_asset_id = ' || p_asset_id ||
                        ' ,p_pl_id = ' || p_pl_id || ' ,p_due_date = ' || p_due_date ||
                        ' ,p_is_period = ' || p_is_period || ' ,p_start_per_date = ' ||
                        p_start_per_date || ' ,p_end_per_date = ' || p_end_per_date);
    ...
    xx_pkg_log.save_log('calc_prodline_fee_delta - End, (v_curr_sum, v_prev_sum) - ' || v_curr_sum || ' ;' ||
                        v_prev_sum);
  ...
  BEGIN
    xx_pkg_log.save_log('create_paymnt_by_templ Enter, p_payment_templ_id = ' || p_payment_templ_id ||
                        ' ,p_payment_terms_id = ' || p_payment_terms_id || ' ,p_number = ' ||
                        p_number || ' ,p_date = ' || p_date || ' ,p_grace_date = ' || p_grace_date ||
                        ' ,p_amount = ' || p_amount || ' ,p_fund_id = ' || p_fund_id ||
                        ' ,p_rev_amount = ' || p_rev_amount || ' ,p_rev_fund_id = ' || p_rev_fund_id ||
                        ' ,p_rev_rate_type_id = ' || p_rev_rate_type_id || ' ,p_rev_rate = ' ||
                        p_rev_rate || ' ,p_contact_id = ' || p_contact_id ||
                        ' ,p_contact_bank_acc_id = ' || p_contact_bank_acc_id ||
                        ' ,p_company_bank_acc_id = ' || p_company_bank_acc_id || ' ,p_note = ' ||
                        p_note || ' ,p_primary_doc_id = ' || p_primary_doc_id || ' ,p_plan_date = ' ||
                        p_plan_date);
    ...
    BEGIN
      ...
      xx_pkg_log.save_log('create_paymnt_by_templ End');
    EXCEPTION
      WHEN OTHERS THEN
        ...
        ELSE
          ...
          LOOP
            ...
            v_total_amount := v_total_amount + get_last_premium_amount(rec.pol_header_id) +
                              get_last_ret_amount(rec.pol_header_id) -
  ...
  (
   ...
   ,p_fee_payment_term    NUMBER DEFAULT 1
    ...
    v_is_period           NUMBER;
  BEGIN
    xx_pkg_log.save_log('create_paymnt_sheduler Enter, p_payment_templ_id = ' || p_payment_templ_id ||
                        ' ,p_payment_terms_id = ' || p_payment_terms_id || ' ,p_total_amount = ' ||
                        p_total_amount || ' ,p_fund_id = ' || p_fund_id || ' ,p_rev_fund_id = ' ||
                        p_rev_fund_id || ' ,p_rev_rate_type_id = ' || p_rev_rate_type_id ||
                        ' ,p_rev_rate = ' || p_rev_rate || ' ,p_contact_id = ' || p_contact_id ||
                        ' ,p_contact_bank_acc_id = ' || p_contact_bank_acc_id ||
                        ' ,p_company_bank_acc_id = ' || p_company_bank_acc_id || ' ,p_first_date = ' ||
                        p_first_date || ' ,p_primary_doc_id = ' || p_primary_doc_id ||
    ...
    LOOP
      ...
      v_cur_num     := v_cur_num + 1;
    END LOOP;
    xx_pkg_log.save_log('create_paymnt_sheduler End, v_cur_num = ' || v_cur_num);
    NULL;
  END;
  ...
  BEGIN
    ...
    RETURN v_amount;
    xx_pkg_log.save_log('get_pol_charge_amount(' || p_ph_id || ') = ' || v_amount);
  END;
  ...
  PROCEDURE create_paymnt_sheduler_renlife(p_p_policy_id NUMBER) IS
    ...
    v_next_next_due_date  DATE; --Дата следующего план графика после v_next_due_date
  BEGIN
    xx_pkg_log.save_log('create_paymnt_sheduler_renlife - Enter, p_p_policy_id = ' || p_p_policy_id);
    ...
    xx_pkg_log.save_log('create_paymnt_sheduler_renlife v_payment_terms_id = ' || v_payment_terms_id ||
                        ' ,v_fund_id = ' || v_fund_id || ' ,v_head_start_date = ' ||
                        v_head_start_date || ' ,v_start_date = ' || v_start_date ||
                        ' ,v_first_date = ' || v_first_date || ' ,v_is_period = ' || v_is_period ||
                        ' ,v_num_of_pay = ' || v_num_of_pay || ' ,v_num_of_years = ' ||
                        v_num_of_years || ' ,v_grace_period = ' || v_grace_period ||
                        ' ,v_pol_header_id = ' || v_pol_header_id || ' ,v_is_group = ' || v_is_group ||
    ...
    THEN
      ...
      BEGIN
        ...
        END IF;
        xx_pkg_log.save_log('create_paymnt_sheduler_renlife, v_changed_cnt = ' || v_changed_cnt);
      END;
    END IF;
    -- Групповые договора. Счет на доплату.Получаю дату начала периода оплаты в который попадает
    -- дата версии
    v_period_date := get_period_date(v_head_start_date, v_num_of_pay, v_start_date);
    ...
    SELECT COUNT(*)
     ...
     WHERE dd.parent_id = p.policy_id
       ...
       AND (p.is_group_flag = 1 OR d.plan_date >= p.start_date); --У групповых договоров даты могут быть где угодно.
    --       AND d.plan_date >=p.start_date;
  
    xx_pkg_log.save_log('create_paymnt_sheduler_renlife, v_acc_cnt = ' || v_acc_cnt);
    IF v_acc_cnt = 0
    THEN
    ...
    v_start_plan_date := get_period_date(v_head_start_date, 1, v_start_date);
    ...
    LOOP
      ...
      LOOP
        ...
        v_next_next_due_date := ADD_MONTHS(v_first_date
                                          ,nvl(v_months_diff, 0) + (v_year_count - v_curr_year) * 12) +
                                nvl(v_days_diff, 0);
        ...
        BEGIN
          ...
          xx_pkg_log.save_log('create_paymnt_sheduler_renlife, <<add_acc_created>>');
          <<add_acc_created>>
          ...
          calc_fee_delta(p_p_policy_id
                        ...
                        ,v_is_period
                        ,NULL
                        ,v_next_next_due_date);
        
          xx_pkg_log.save_log('create_paymnt_sheduler_renlife, <<create_acc>>');
          <<create_acc>>
      ...
      END LOOP;
    END LOOP;
    xx_pkg_log.save_log('create_paymnt_sheduler_renlife - End');
     ...
     ,cp_is_first   NUMBER
      ...
      SELECT dsa.obj_uro_id oper_templ_id
       ...
       WHERE ass.p_policy_id = p.policy_id
         ...
         AND (p.is_group_flag = 1 AND sh.brief = 'NEW' --для групповых договоров берём только новые записи
             OR nvl(p.is_group_flag, 0) = 0 AND c.start_date <= cp_doc_date AND
             c.end_date >= cp_doc_date)
  ...
  BEGIN
    IF get_current_user <> 'PMIKUSHIN'
    THEN
      RETURN;
    END IF;
    --savepoint ddd;
    xx_pkg_log.save_log('set_payment_status - Enter, doc_id = ' || doc_id);
    hashmap.delete;
    ...
    xx_pkg_log.save_log('v_payment_due_date = ' || v_payment_due_date);
    xx_pkg_log.save_log('v_doc_status_id = ' || v_doc_status_id);
    ...
    xx_pkg_log.save_log('v_Doc_Date = ' || v_doc_date || ' |v_Is_First = ' || v_is_first);
    ...
    THEN
      ...
      xx_pkg_log.save_log('v_Company_Brief = ' || v_company_brief || ' |v_is_storno = ' ||
                          v_is_storno);
      ...
      END;
      xx_pkg_log.save_log('v_Parent_Ent_Brief = ' || v_parent_ent_brief || ' |v_Doc_Templ_Brief = ' ||
                          v_doc_templ_brief);
        ...
        THEN
          ...
          CASE
            WHEN v_doc_templ_brief = 'PAYMENT'
              ...
              xx_pkg_log.save_log('OPEN c_Oper_Renlife(' || v_doc_status.src_doc_status_ref_id || ', ' ||
                                  v_doc_status.doc_status_ref_id || ', ' || v_doc_date || ', ' ||
                                  v_is_first || ')');
          ...
          LOOP
            CASE
              WHEN v_doc_templ_brief = 'PAYMENT'
                ...
                xx_pkg_log.save_log('1 v_Oper_Templ_ID = ' || v_oper_templ_id || ', v_Is_Self = ' ||
                                    v_is_self || ', v_Is_Accepted = ' || v_is_accepted ||
                                    ', v_Cover_Ent_ID = ' || v_cover_ent_id || ', v_Cover_Obj_ID = ' ||
                                    v_cover_obj_id || ', v_Real_Amount = ' || v_real_amount ||
                                    ', v_Plan_Amount = ' || v_plan_amount || ', v_Self_Amount = ' ||
                                    v_self_amount || ', v_Set_Off_Amount = ' || v_set_off_amount);
              
                -- Ф.Ганичев. По-другому считает сумму уже начисленного по покрытию
                v_plan_amount := utils.get_null(hashmap, 'ACC_' || v_cover_obj_id);
                xx_pkg_log.save_log('1 v_Plan_Amount = ' || v_plan_amount);
          ...
          xx_pkg_log.save_log('v_Count = ' || v_count || ' |v_Amount = ' || v_amount);
          ...
          LOOP
            CASE
              WHEN v_doc_templ_brief = 'PAYMENT'
                ...
                xx_pkg_log.save_log('2 v_Oper_Templ_ID = ' || v_oper_templ_id || ', v_Is_Self = ' ||
                                    v_is_self || ', v_Is_Accepted = ' || v_is_accepted ||
                                    ', v_Cover_Ent_ID = ' || v_cover_ent_id || ', v_Cover_Obj_ID = ' ||
                                    v_cover_obj_id || ', v_Real_Amount = ' || v_real_amount ||
                                    ', v_Plan_Amount = ' || v_plan_amount || ', v_Self_Amount = ' ||
                ...
                v_plan_amount := utils.get_null(hashmap, 'ACC_' || v_cover_obj_id);
                xx_pkg_log.save_log('2 v_Plan_Amount = ' || v_plan_amount);
                IF v_plan_amount IS NULL
                THEN
                  v_plan_amount := get_charge_prodline_amount(v_cover_obj_id, v_payment_due_date, v_payment_due_date)
                                   .fund_amount;
            ...
            xx_pkg_log.save_log('v_Is_Storno = ' || v_is_storno || ' |v_Real_Amount = ' ||
                                v_real_amount || ' |v_Plan_Amount = ' || v_plan_amount);
            ...
            THEN
              ...
              THEN
                ...
                xx_pkg_log.save_log(id || ' := Acc_New.Run_Oper_By_Template(v_Oper_Templ_ID = ' ||
                                    v_oper_templ_id || ' ,doc_id = ' || doc_id ||
                                    ' ,v_Cover_Ent_ID = ' || v_cover_ent_id || ' ,v_Cover_Obj_ID = ' ||
                                    v_cover_obj_id || ' ,v_doc_status.doc_status_ref_id = ' ||
                                    v_doc_status.doc_status_ref_id || ' ,1 ' || ' ,v_Oper_Amount = ' ||
                                    v_oper_amount || ')');
                  ...
                  THEN
                    ...
                    LOOP
                      ...
                      ELSE
                        v_key := utils.get_null(ag_comm_cache
                                               ,'AG_COMM_VKEY' || v_pac_id || ' ' ||
                                                pkg_agent_rate.date_pay);
                      END IF;
                      xx_pkg_log.save_log('v_key = ' || v_key);
                      IF v_key = 0
                      THEN
                      ...
                      ELSE
                        v_comm_amt := v_st;
                      END IF;
                      xx_pkg_log.save_log('v_comm_amt = ' || v_comm_amt);
                      IF v_comm_amt > 0
                      THEN
                        ...
                        id := acc_new.run_oper_by_template(v_oper_templ_id
                                                          ...
                                                          ,1
                                                          ,v_comm_amt);
                        xx_pkg_log.save_log(id ||
                                            ' := Acc_New.Run_Oper_By_Template(v_Oper_Templ_ID = ' ||
                                            v_oper_templ_id || ' ,doc_id = ' || doc_id ||
                                            ' ,v_Cover_Ent_ID = ' || v_cover_ent_id ||
                                            ' ,v_Cover_Obj_ID = ' || v_cover_obj_id ||
                                            ' ,v_doc_status.doc_status_ref_id = ' ||
                                            v_doc_status.doc_status_ref_id || ' ,1 = ' ||
            ...
            ELSE
              ...
              THEN
                xx_pkg_log.save_log('v_Storno_Amount = ' || v_storno_amount);
                id := acc_new.run_oper_by_template(v_oper_templ_id
                                                  ,doc_id
                                                  ,v_cover_ent_id
                                                  ,v_cover_obj_id
      ...
      END;
    END IF;
    xx_pkg_log.save_log('set_payment_status - End');
    --rollback to ddd;
  EXCEPTION
    WHEN OTHERS THEN
      xx_pkg_log.save_log('set_payment_status - Error');
      ROLLBACK TO ddd;
  END;
  ...
  BEGIN
    ...
    THEN
      ...
      EXCEPTION
        WHEN OTHERS THEN
          raise_application_error(-20100
                                 ,'Ошибка при получении зачета: ' || SQLERRM);
      END;
  ...
  */
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        v_result := 0;
    END;
    xx_pkg_log.save_log('get_last_ret_amount(' || p_pol_header_id || ') = ' || v_result);
    RETURN v_result;
  END;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        v_result := 0;
    END;
    xx_pkg_log.save_log('GET_PLAN_RET_AMOUNT(' || p_pol_header_id || ') = ' || v_result);
    RETURN v_result;
  END;
  ...
  BEGIN
    xx_pkg_log.save_log('POLICY_MAKE_PLANNING - Enter, p_p_policy_id = ' || p_p_policy_id ||
                        ' ,p_Payment_Term_ID = ' || p_payment_term_id || ' ,p_planning_type' ||
                        p_planning_type);
    ...
    log(p_p_policy_id, 'POLICY_MAKE_PLANNING COMPANY_BRIEF ' || v_company_brief);
    ...
    THEN
      ...
      ELSE
        ...
        IF get_plan_amount(v_policy.pol_header_id) > 0
        THEN
          v_payment_id := NULL;
        END IF;
      END IF;
      ...
      THEN
        ...
        END IF;
        xx_pkg_log.save_log('POLICY_MAKE_PLANNING, v_Company_Brief = ' || v_company_brief);
        IF v_company_brief = 'RenLife'
        THEN
        
          log(p_p_policy_id, 'POLICY_MAKE_PLANNING VALIDATE_POL_SETOFF_SUM ');
          validate_pol_setoff_sum(p_p_policy_id);
        
          log(p_p_policy_id, 'POLICY_MAKE_PLANNING CREATE_PAYMNT_SHEDULER_RENLIFE ');
          create_paymnt_sheduler_renlife(p_p_policy_id);
        ELSE
          ...
          IF v_pol_charge_amount = 0
             AND doc.get_last_doc_status_brief(p_p_policy_id) = 'PROJECT'
             AND pkg_policy.is_addendum(p_p_policy_id) = 0
          THEN
          ...
          THEN
            ...
            create_paymnt_sheduler(v_payment_templ_id
                                  ,v_payment_term_id
                                  ,v_total_amount
                                  ,v_fund_id
                                  ,v_fund_pay_id
                                  ...
                                  ,ent.calc_attr('P_POLICY', 'POLICY_HOLDER_ID', v_policy.policy_id)
                                  ,NULL
                                  ,v_company_bank_acc_id
                                  ,
                                   --v_policy.start_date,
    ...
    THEN
      ...
      /*!!!!!!!!!!!!!!!!
            IF get_plan_ret_amount(v_policy.pol_header_id) =
               get_last_ret_amount(v_policy.pol_header_id) THEN
              v_Payment_ID := 1;
            ELSE
              v_Payment_ID := NULL;
            END IF;
      ...
      xx_pkg_log.save_log('POLICY_MAKE_PLANNING, v_Payment_ID = ' || v_payment_id);
      ...
      THEN
        ...
        xx_pkg_log.save_log('POLICY_MAKE_PLANNING v_Total_Amount = ' || v_total_amount ||
                            ', v_payment_term_id = ' || v_payment_term_id ||
                            ', v_Payment_Templ_ID = ' || v_payment_templ_id ||
                            ', v_Payment_Term_ID = ' || v_payment_term_id || ', v_fund_id = ' ||
                            v_fund_id || ', v_fund_pay_id = ' || v_fund_pay_id ||
                            ', v_Rate_Type_ID = ' || v_rate_type_id || ', v_Company_Bank_Acc_ID = ' ||
                            v_company_bank_acc_id || ', v_policy.start_date = ' ||
                            v_policy.start_date);
      ...
      END IF;
    END IF;
    xx_pkg_log.save_log('POLICY_MAKE_PLANNING - End');
  END;
  ...
  BEGIN
    ...
    THEN
      ...
      LOOP
        v_plan_amount    := v_plan_amount + get_plan_amount(rec.child_id);
      ...
      THEN
        ...
        LOOP
          v_total_amount := v_total_amount + get_last_premium_amount(rec.child_id) +
                            get_last_ret_amount(rec.child_id) - get_plan_amount(rec.child_id);
        END LOOP;
        ...
        THEN
          ...
          create_paymnt_sheduler(v_payment_templ_id
                                ...
                                ,v_rate_type_id
                                ,CASE WHEN v_gen_policy.fund_id = v_gen_policy.fund_pay_id THEN 1.0 ELSE CASE WHEN
                                 acc_new.get_rate_by_id(v_rate_type_id
                                                       ,v_gen_policy.fund_pay_id
  ...
  */
  ...
  BEGIN
    ...
    LOOP
      ...
      -- Ф.Ганичев
      v_paid_amount   := get_pay_cover_amount(rec.p_cover_id).fund_amount;
      v_charge_amount := get_charge_cover_amount(rec.p_cover_id).fund_amount;
  ...
  */
  ...
  BEGIN
    ...
    policy_make_planning(p_policy_id, v_payment_term_id, 'PAYMENT');
  
    policy_make_planning(p_policy_id, nvl(v_paymentoff_term_id, v_payment_term_id), 'PAYORDBACK');
  ...
  (
   ...
   ,p_end_date   DATE
    ...
    v_pol_id          NUMBER;
  BEGIN
    xx_pkg_log.save_log('get_charge_prodline_amount - Enter, p_p_cover_id = ' || p_p_cover_id ||
    ...
    SELECT a.p_asset_header_id
     ...
     WHERE pc.p_cover_id = p_p_cover_id
       ...
       AND a.p_policy_id = p.policy_id
       AND p.pol_header_id = ph.policy_header_id;
    xx_pkg_log.save_log('v_asset_header_id = ' || v_asset_header_id || ', v_plo_id = ' || v_plo_id ||
                        ', v_fund_id = ' || v_fund_id || ', v_pol_id = ' || v_pol_id);
    /*
    ...
    */
    ...
    xx_pkg_log.save_log('get_charge_prodline_amount - End v_sum = ' || v_sum);
    RETURN v_ret_val;
  END;
                            ...
                            ,CASE
                               WHEN (p_payment_type_id = 0) THEN
                                ROUND((t.amount + t.comm_amount -
                                      get_paym_set_off_amount(t.payment_id, dso.doc_set_off_id)) /
                                      dso.set_off_rate * 100) / 100
                               WHEN (p_payment_type_id = 1) THEN
                                ROUND((t.amount + t.comm_amount -
                                      get_bill_set_off_amount(t.payment_id, dso.doc_set_off_id)) *
                                      dso.set_off_rate * 100) / 100
                             END main_amount
                            ,CASE
                               WHEN (p_payment_type_id = 0) THEN
                ...
                FROM (SELECT /*+ NO_MERGE (t) */
                      ...
                      ,CASE
                         WHEN (p_payment_type_id = 0) THEN
                          CASE
                            WHEN p_fund_id_bill = p_rev_fund_id THEN
                             t.amount + t.comm_amount - get_paym_set_off_amount(t.payment_id, 0)
                            ELSE
                             CASE
                               WHEN p_fund_id_bill = v_native_fund_id THEN
                                ROUND((t.amount + t.comm_amount - get_paym_set_off_amount(t.payment_id, 0)) *
                                      (CASE
                                        WHEN p_rev_rate_type_brief = 'ФИКС' THEN
                                         p_rev_rate
                                        ELSE
                         ...
                         WHEN (p_payment_type_id = 1) THEN
                          CASE
                            WHEN t.fund_id = t.rev_fund_id THEN
                             t.amount + t.comm_amount - get_bill_set_off_amount(t.payment_id, 0)
                            ELSE
                             CASE
                               WHEN t.fund_id = v_native_fund_id THEN
                                ROUND((t.amount + t.comm_amount - get_bill_set_off_amount(t.payment_id, 0)) /
                                      (CASE
                                        WHEN rt.brief = 'ФИКС' THEN
                                         t.rev_rate
                                        ELSE
  ...
  BEGIN
    IF nvl(get_set_off_amount(p_doc_id, NULL, NULL), 0) != 0
    THEN
      raise_application_error(-20100
