CREATE OR REPLACE PACKAGE Pkg_Agent_1 IS
  ...
  */
  ...
  DATE_START             DATE := SYSDATE; -- переменная для использования в представлении v_agent_current_status
  glb_clear_flag_attest  VARCHAR2(1) := 'Д'; -- чистка темповой таблицы по расчету СГП
  /**
  ...
  */
  ...
  /**
  * Определя.т на дату привязанного агента к договору
  * Веселуха Е.В. 19-06-2009
  **/
  ...
  /**
  * Определя.т ФИО привязанного агента к договору
  * Веселуха Е.В. 14-09-2009
  **/
  ...
  */
  ...
  /*Каткевич А.Г. Правильный подсчет СГП для ДАВР*/
  ...
  * Производит расчет коэффициентов по всем агентам/менеджерам
   ...
   *
   * @author Шидловская Татьяна
   * @param p_header_id заголовок агентского договора, если =0 , то по всем договорам
  ...
  */
  ...
  /**
  * Определяет количество оплачиваемых лет по Договору страхования.
  * Используется в форме AG_REPORT_DET = поле NUM_YEAR
  * @author Шидловская Татьяна
  */
  FUNCTION get_num_year_OAV(p_policy_header_id IN NUMBER) RETURN NUMBER;

  /**
  * вспомогательная процедура для использования в представлении
  * @author Шидловская Татьяна
  */
  ...
  ------------------------------------------------------------
  --
  -- Расчитывает СГП по рекрутированным Агентам
  --
  ------------------------------------------------------------
  ...
  ------------------------------------------------------------
  --
  -- Расчитывает кол-во рекрутированных Агентов
  --
  ------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY Pkg_Agent_1 IS
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        RAISE_APPLICATION_ERROR(err
                               ,'Не обновляется статус действующей версии договора. ' || SQLERRM(err));
    END;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RAISE_APPLICATION_ERROR(-20001
                             ,'Ошибка обновления Департамента. (p_contr_id=' || p_contr_id ||
  ...
  BEGIN
    ...
    --Каткевич А.Г. 29/11/2009 - отказался от подключения к документам, но возможны ошибки
  ...
  */
  ...
  BEGIN
    SELECT DISTINCT NVL(pc.fee, 0)
      INTO v_sum_adm_izd
      FROM ---p_pol_header ph -- заголовок договора страхования
           P_POLICY pp --on (ph.policy_id = pp.policy_id) -- текущая версия доровора страхования
      JOIN AS_ASSET ass
        ON (ass.p_policy_id = pp.policy_id) -- объект по договору страхования
      JOIN P_COVER pc
        ON (pc.as_asset_id = ass.as_asset_id) -- покрытие по договору страхования
  ...
  BEGIN
    ...
    BEGIN
      IF NVL(glb_clear_flag_attest, 'Д') = 'Д'
      THEN
      ...
      /*  select SQ_AG_STAT_DET_TMP.nextval AG_STAT_DET_TMP_ID, 0 , attest.*
        from
        (
        select   0                                            -- иди договора менеджера
          ...
          FROM (SELECT /*distinct*/
                ...
                ,(CASE
                   ...
                   WHEN (pt.brief = 'Единовременно' AND
                        ...
                        OR pt.brief <> 'Единовременно' THEN
                    1
                   ELSE
                    1
                 END) koef
                ...
                ,
                 ...
                 WHERE ph.policy_id = pp.policy_id
                   ...
                   AND pkg_agent_sub.get_p_agent_status_by_date(p_ag_contract_header_id
                                                               ,ph.policy_header_id
                                                               ,p_end_date) IN ('CURRENT') -- статус агента по договору страхования
  ...
  END;
  -- Рассчитывает коэффициент СГП по одному договору агента с формированием ТМП таблицы
  -- Это!!! ничем не отличается от attest_agent_SGP. только коэффициентами и парочкой условий
  -- вычисляет СГП для ДАВ (тип ДАВ)
  PROCEDURE attest_agent_SGP_new
  (
    p_ag_contract_header_id IN NUMBER
   ,p_start_date            DATE
  ...
  -- Каткевич А.Г. 18.09.2008 Добавил процедуру для подсчета СГП по ДАВР
  PROCEDURE attest_agent_SGP_DAVR
  (
    p_ag_contract_header_id IN NUMBER
   ,p_start_date            DATE
  ...
  BEGIN
    ...
    BEGIN
      ...
      INSERT INTO AG_STAT_DET_TMP
        ...
        --               koef_prikaz -- сумма премии минус издержки с учетом коэффициентов
          FROM (SELECT (CASE -- коэффициент доли агента при заключении договора
                         WHEN (ar.brief = 'PERCENT') THEN
                          pa.part_agent / 100 -- процент
                         WHEN (ar.brief = 'ABSOL') THEN
  ...
  (
   ...
   ,p_start_date            IN DATE
   , -- дата начала оцениваемого периода
    --                                   p_end_date              in date, -- дата конца оцениваемого периода
    p_K_SGP OUT NUMBER
   ,p_K_KD  OUT NUMBER
   ,p_K_KSP OUT NUMBER
  ) IS
  ...
  BEGIN
    ...
    BEGIN
      ...
      -- ужас. времени нет
      FOR recrut IN ( -- существует хотябы одна запись, где в графе рекрутер - нужный человек (см ниже)
        ...
        INSERT INTO AG_STAT_DET_TMP
          ...
          SELECT SQ_AG_STAT_DET_TMP.NEXTVAL AG_STAT_DET_TMP_ID
            ...
            FROM (SELECT (CASE -- коэффициент доли агента при заключении договора
                   ...
                   WHERE ph.policy_id = pp.policy_id -- текущая версия доровора
                     AND ph.policy_header_id = pa.policy_header_id -- aгенты по договору страхования
                        --and ppc.policy_id = ph.policy_id
                        /*and tcp.id = ppc.contact_policy_role_id
                        and tcp.brief = 'Страхователь'*/
                     AND ch.ag_contract_header_id = pa.ag_contract_header_id
                     AND pa.ag_contract_header_id = ah.ag_contract_header_id -- договор с агентом
                     AND chan.ID = ah.t_sales_channel_id
                     AND chan.brief IN ('BR', 'BANK') -- канал продаж "БАНК", "Брокер"
                     AND ah.last_ver_id = ac.ag_contract_id -- версия договора с агентом
                     AND pa.status_id = pas.policy_agent_status_id -- статусы агентов по договору страхования
                     AND pa.ag_type_rate_value_id = ar.ag_type_rate_value_id -- тип доли агента в договоре
                     AND pt.ID = pp.payment_term_id
                     AND pas.brief IN ('CURRENT')
                     AND EXISTS (SELECT 1
                            FROM ven_p_policy_contact   ppc
                     ...
                     AND EXISTS (SELECT 1
                        ...
                        -- существует хотябы одна запись, где в графе рекрутер - нужный человек
                     AND ac.contract_recrut_id = recrut.ag_contract_id);
      END LOOP;
      /*from p_pol_header ph
        join p_policy pp on (ph.policy_id = pp.policy_id) -- текущая версия доровора
       -- join ven_doc_doc d on (d.parent_id = pp.policy_id) -- связь документов для анализа оплаты
       -- join ven_ac_payment ap on (ap.payment_id =
        ...
        join ag_contract_header ah on (pa.ag_contract_header_id =
                                      ah.ag_contract_header_id) -- договор с агентом
        join ven_t_sales_channel chan on (chan.id =
                                         ah.t_sales_channel_id and
                                         chan.brief in
                                         ('BR', 'BANK')) -- канал продаж "БАНК", "Брокер"
        join ag_contract ac on (ah.last_ver_id =
                               ac.ag_contract_id) -- версия договора с агентом
      ...
      LOOP
        ...
        LOOP
          ...
          THEN
            --- Что это?????? это так определяете смену значения???
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20000
                               ,'Ошибка расчета СПГ(Банк) для p_ag_contract_header_id: ' ||
                                p_ag_contract_header_id || ' ' || SQLERRM(SQLCODE));
  ...
  BEGIN
    ...
    BEGIN
        ...
        SELECT SQ_AG_STAT_DET_TMP.NEXTVAL AG_STAT_DET_TMP_ID
          ...
          FROM (SELECT (CASE -- коэффициент доли агента при заключении договора
                ...
                \*                            and ch.ag_contract_header_id =
                  ...
                  JOIN ven_t_sales_channel chan
                    ON (chan.ID = ch.t_sales_channel_id AND chan.brief = 'MLM') -- агентский канал продаж
                 ...
                 WHERE 1 = 1
                   ...
                   AND EXISTS (SELECT '1'
                         ...
                         WHERE tcp.ID = ppc.contact_policy_role_id
                           ...
                           AND ppc.policy_id = pp.policy_id
                           AND ch.agent_id <> ppc.contact_id)
                      -- существует зависимость версии агентского договора от агентства на дату оплаты
                   AND EXISTS
                 (SELECT '1'
                          FROM ven_ag_contract cc
                         WHERE cc.agency_id = v_agency
                           AND cc.contract_id <> p_ag_contract_header_id -- не сам директор
                           AND cc.contract_id = ch.ag_contract_header_id
                              -- дата заключения договора меньше или равна дате оплаты
                           AND cc.date_begin <= pay.dt
                              -- на дату оплаты версия договора была действующая
                           AND Doc.get_doc_status_brief(cc.ag_contract_id, pay.dt) = 'CURRENT'
                           AND --следующая версия договора заключена позже чем оплата А7 
                               (EXISTS
                                (SELECT '1'
                               ...
                               -- или это есть максимальная версия договора
      ...
      /*              from p_pol_header ph
                    ...
                    --                           d.child_id) -- проверка оплаты по документу выше
                      ...
                      join ven_t_sales_channel chan on (chan.id =
                                                       ah.t_sales_channel_id and
                                                       chan.brief = 'MLM') -- агентский канал продаж
                      join ag_contract ac on (ah.last_ver_id = ac.ag_contract_id) -- версия договора с агентом
                      join policy_agent_status pas on (pa.status_id =
                                                      pas.policy_agent_status_id) -- статусы агентов по договору страхования
                      join ag_type_rate_value ar on (pa.ag_type_rate_value_id =
                                                    ar.ag_type_rate_value_id) -- тип доли агента в договоре
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20000
                               ,'Ошибка расчета СПГ (Директор) для p_ag_contract_header_id: ' ||
                                p_ag_contract_header_id || ' ' || SQLERRM(SQLCODE));
  ...
  BEGIN
    ...
    BEGIN
      ...
      -- select t.ID into t_pay_terms_edin from  t_payment_terms t where t.brief = 'Единовременно';
      ...
      INSERT INTO AG_STAT_DET_TMP
        ...
        SELECT SQ_AG_STAT_DET_TMP.NEXTVAL AG_STAT_DET_TMP_ID
              ,0
              ,p_ag_contract_header_id -- иди договора менеджера
              ,AGENT -- иди договора агента, подчиненного менеджеру
              ,policy_id -- заголовок договора страхования
              ,agent_rate * koef * koef_GR * ((koef_ccy * sum_prem) - sum_izd /** koef_prikaz*/
               ) -- сумма премии минус издержки с учетом коэффициентов
          FROM (SELECT (CASE -- коэффициент доли агента при заключении договора
                         WHEN (ph.AG_TYPE_RATE_VALUE_ID = v_ag_type_rate_percent) THEN
                          ph.part_agent / 100 -- процент
                         WHEN (ph.AG_TYPE_RATE_VALUE_ID = v_ag_type_rate_ABSOL) THEN
                          ph.part_agent -- абсолютная величина
                         ELSE
                          ph.part_agent -- в противном случае доля агента, как абсолютная величина
                       END) agent_rate
                      ,(CASE
                         WHEN (ph.payment_term_BRIEF = 'Единовременно' AND
                         ...
                         WHEN (((ph.payment_term_BRIEF = 'Единовременно') AND
                              ...
                              ph.payment_term_BRIEF <> 'Единовременно') THEN
                          1
                         ELSE
                          1
                       END) koef
      ...
      /*
          exception
            when others then
              raise_application_error(-20000, 'Ошибка расчета СПГ(Менеджер) для p_ag_contract_header_id: '||p_ag_contract_header_id||' '||sqlerrm(sqlcode));
      */
      --v_K_SGP := 0;
    END;
  ...
  (
   ...
   ,p_K_KSP                 IN NUMBER
   ,p_categor_id            IN NUMBER
   ,p_status_id             IN OUT NUMBER -- передается значение = предыдущему статусу
  ) IS
  ...
  BEGIN
    ...
    BEGIN
      SELECT 1
        INTO v_rez_agent
        FROM dual
       WHERE EXISTS -- даты не изменяемых статусов, и категории гарантированно не понижаемых статусов.
       (SELECT '1'
                FROM ven_ag_stat_agent sa
                    ,ven_ag_stat_hist  sh
               WHERE sa.brief IN ('ВедКонс', 'ФинСов', 'ВедМенед')
                 ...
                 AND sh.ag_stat_agent_id = p_status_id) -- статус соответствует последнему назначенному               
          OR (EXISTS -- если в течении 6 месяцев не было понижения с 'ВедМенед' до 'СтМенед'
              (SELECT '1'
                 FROM ven_ag_stat_agent sa
                     ,ven_ag_stat_hist  sh
                WHERE sa.brief IN ('ВедМенед')
                  ...
                  AND sh.ag_category_agent_id = p_categor_id
                  AND sh.stat_date BETWEEN ADD_MONTHS(LAST_DAY(p_start_date) + 1, -7) AND p_start_date
                  AND NOT EXISTS (SELECT '1' -- дата изменения статуса позже, чем дата назначение 'ВедМенед'
                         FROM ven_ag_stat_agent sa1
                             ,ven_ag_stat_hist  sh1
                        WHERE sa1.brief IN ('СтМенед')
    ...
    EXCEPTION
      WHEN OTHERS THEN
        v_rez_agent := v_rez_agent; -- статус можно менять
    END;
    ...
    BEGIN
      SELECT 2
        INTO v_rez_agent
        FROM dual
       WHERE (EXISTS --3.5 года не изменялся статус
              (SELECT '1'
                 FROM ven_ag_stat_agent sa
                     ,ven_ag_stat_hist  sh
                WHERE sa.brief IN ('ФинСов')
                  ...
                  AND NOT EXISTS (SELECT '1' -- не существует записи с другим статусом и датой позже 
                         FROM ven_ag_stat_agent sa1
                             ,ven_ag_stat_hist  sh1
                        WHERE sa1.ag_stat_agent_id = sh1.ag_stat_agent_id
                          ...
                          AND sh1.stat_date > sh.stat_date
                          AND sh1.stat_date <= p_start_date
                          AND sh1.ag_stat_agent_id <> sh.ag_stat_agent_id)) OR EXISTS --2.5 года не изменялся статус
              (SELECT '1'
                 FROM ven_ag_stat_agent sa
                     ,ven_ag_stat_hist  sh
                WHERE sa.brief IN ('ВедМенед')
    ...
    THEN
      ...
      IF v_rez_agent = 0
      THEN
        -- проверка статуса на уменьшение (можно уменьшать)
   ...
   ,p_start_date  IN OUT DATE
    ...
    /*      select vv.*
      ...
      SELECT ah.ag_contract_header_id AS HEAD_ID
        ...
        FROM ven_ag_contract_header ah
      -- версия договора на заданную дату
      ...
      -- версия договора, присвоения категории
        JOIN ven_ag_contract ac_cat
          ON (ac_cat.contract_id = ah.ag_contract_header_id)
      -- статус агента на дату (если существует)
  ...
  BEGIN
    ...
    LOOP
      ...
      THEN
        ...
        END IF;
        -- заполнение временной таблицы общей детализации по аттестации // временная табл.детализации по одному агенту очищается
  ...
  -- Возвращает бриф категории агента
  FUNCTION get_agent_cat_brief_by_date
  (
    contract_header_id NUMBER
   ,status_date        DATE
  ...
  BEGIN
    ...
    --Каткевич А.Г. 29/01/20009 теперь берется только 1ый ЭПГ и берется его макс дата
    --26/03/2009 если есть дата платежного документа то возвращаем только ее (раньше возвращали
    -- меншьую из пары дат: договора и платежного документа)
    --14/04/2009 Новые требования теперь берем большую из дат.
    --для того чтобы при отсутсвии оплаты взять дату начала ДС указываем параметр=2
    --02/05/2009 Добавил поддержку старого типа расчета (параметр=1)
    CASE
        ...
        SELECT GREATEST(NVL(( /*SELECT MAX(pa2.reg_date) date_a7
                            ...
                            SELECT MAX(pa2.reg_date)
                             ...
                             WHERE ph.policy_header_id = pph.policy_header_id
                               ...
                               AND ap.payment_id = d.child_id
                                  -- AND Doc.get_doc_status_brief(ap.payment_id) = 'PAID'
                                  --Каткевич А.Г. 04/06/2009 не учитываем анулированные платежи
                               AND doc.get_doc_status_brief(pa2.payment_id) <> 'ANNULATED'
                               AND dso.parent_doc_id = ap.payment_id
                               AND dso.child_doc_id = pa2.payment_id
                               AND (acpt.brief = 'A7' OR acpt.brief = 'PD4' OR acpt.brief = 'ПП')
                                  --Каткевич А.Г. 04/06/2009 Если 1 ЭПГ не в статусе ОПЛАЧЕН ищем минимальный оплаченый с такой же датой
      ...
      WHEN par_return = 5 THEN
        --Условие вступления договора в силу (используется в мотиваци)
        BEGIN
          SELECT MAX(pay_doc.due_date)
           ...
           WHERE 1 = 1
             ...
             AND epg.doc_templ_id = 4 -- ЭПГ
             AND epg.payment_id = dso.parent_doc_id
             AND doc.get_doc_status_brief(dso.doc_set_off_id) <> 'ANNULATED' -- Документ зачета не анулирован
             AND dso.child_doc_id = pay_doc.payment_id
             AND acpt.payment_templ_id = pay_doc.payment_templ_id
             AND acpt.brief IN ('A7', 'PD4', 'ПП', 'ПП_ПС', 'ПП_ОБ')
                -- AND pay_doc.doc_templ_id IN (6531,5234,86) --А7, ПД4, ПП
                     ...
                     AND acp2.plan_date =
                         (SELECT MIN(acp3.plan_date) --BUG: заявка 24279 
                            FROM ac_payment acp3
                                ...
                                ,p_policy   pp3
                           WHERE 1 = 1
                                --acp3.payment_number =1-- Для мигрированых ДС
                             AND acp3.payment_id = dd3.child_id
                             AND dd3.parent_id = pp3.policy_id
        ...
        END;
      ELSE
        SELECT NVL( --04/06/09 Каткевич А.Г.
                   --В старый вариант изменений не вносил если раньше считали с ошибками то пусть так и будет
                   --Только аннулированый Платежный документ убрал
  ...
  BEGIN
    ...
    if v_date_status not between add_months(sysdate, -1) and sysdate then
      v_mess := v_mess || 'Дата ' || v_date_status ||
                ' не является корректной для данной операции';
      v_err  := v_err + 1;
    end if;*/
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        v_err  := v_err + 1;
    END;
    -- дата действующего статуса документа
    BEGIN
      SELECT s.start_date
        INTO v_date
        FROM DOC_STATUS s
    ...
    THEN
      ...
      v_err  := v_err + 1;
    END IF;
    -- дата статуса "Готов к расторжению"
    v_date := Doc.get_last_doc_status_date(p_ag_contr_id);
  
    IF v_date IS NULL
    THEN
      RAISE_APPLICATION_ERROR(-20000
                             ,'Не найдена дата готовности к расторжению' || SQLERRM);
    END IF;
  
    IF v_date NOT BETWEEN ADD_MONTHS(SYSDATE, -1) AND SYSDATE
    THEN
      v_mess := v_mess || 'Дата ' || v_date || ' не корректная для данной операции';
      v_err  := v_err + 1;
    END IF;
    ...
    THEN
      ...
      BEGIN
        ...
        VALUES
          ...
          ,
           --ADD_MONTHS(v_date, 1),
           --Изменено 02.10.2009 Веселуха Е.В. по заявке №48747
           v_date + 20
      ...
      EXCEPTION
        WHEN OTHERS THEN
          v_mess := 'Проверьте целостность БД по изменению статуса версии АД';
          v_err  := v_err + 1;
      END;
    END IF;
    IF v_err <> 0
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20000
                             ,'Процедура "Готов к расторжению"' || SQLERRM);
  END;
  ----------------------------------------------
  -- Определяет количество оплачиваемых лет по Договору страхования.
  -- Используется в форме AG_REPORT_DET = поле NUM_YEAR
  ...
  BEGIN
    ...
    BEGIN
      -- основной запрпос из формы - нахождение даты оплаты
      SELECT ac.date_pay
        INTO v_day_pay
  ...
  */
  ...
  --
  ------------------------------------------------------------
  -- 28.08.2008 Каткевич А.Г Внес изменения в курсор
  -- 22.09.2008 Каткевич А.Г. Добавил параметр p_start_date - дата начала расчета СГП
  FUNCTION attest_recruit_SGP
  (
    p_ag_contract_id NUMBER
   ,p_end_date       DATE
  ...
  BEGIN
    -- не будем чистить временную таблицу!!!
  
    --DELETE FROM AG_STAT_DET_TMP;
    EXECUTE IMMEDIATE 'TRUNCATE TABLE AG_STAT_DET_TMP';
  
    glb_clear_flag_attest            := 'Н';
                ...
                WHERE EXISTS (SELECT 1
                        ...
                        WHERE pa.status_id = pas.policy_agent_status_id
                          ...
                          AND pa.policy_header_id = ph.policy_header_id
                             -- ограничим договора - периодом расчета.
                          AND Pkg_Agent_1.get_agent_start_contr(ph.policy_header_id) BETWEEN
    ...
    -- теперь нужно чистить временную таблицу!!!
    glb_clear_flag_attest := 'Д';
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20001
                             ,'Ошибка расчета СПГ для рекрутированных агентов для p_ag_contract_id: ' ||
                              p_ag_contract_id || ' См. ошибку Oracle: ' || SQLERRM(SQLCODE));
  ...
  EXCEPTION WHEN OTHERS THEN
     RAISE_APPLICATION_ERROR(-20001, 'Ошибка расчета СПГ для рекрутированных агентов для p_ag_contract_id: '||p_ag_contract_id||' См. ошибку Oracle: '||SQLERRM(SQLCODE));
  ...
  */
  ...
  --
  -- Расчитывает кол-во рекрутированных Агентов
  -- исправление 26.10.2007 Кудрявцев: необходимо определять кол-во ДС заключенных рекрутированными
  --
  -- 22.09.2008 Каткевич А.Г. Добавил p_start_date
  -- 20/02/2009 DEPRICATED Теперь эта функция не сипользуется
  ------------------------------------------------------------
  FUNCTION attest_count_recruit
  (
    p_ag_contract_id NUMBER
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20001
                             ,'Ошибка вычисления кол-ва рекрутированных агентов для p_ag_contract_id: ' ||
                              p_ag_contract_id || ' См. ошибку Oracle: ' || SQLERRM(SQLCODE));
