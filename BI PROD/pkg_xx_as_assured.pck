CREATE OR REPLACE PACKAGE PKG_XX_AS_ASSURED IS
  ...
  FUNCTION get_medical_result
  (
    p_result_brief            IN VARCHAR2
   ,p_assured_medical_exam_id IN NUMBER
  ) RETURN VARCHAR2;

END;
/
CREATE OR REPLACE PACKAGE BODY PKG_XX_AS_ASSURED IS
  ...
  BEGIN
    FOR cur IN (SELECT mer.NOTE
                  FROM MEDICAL_EXAM_RESULT mer
                      ,T_medical_result    tm
                 WHERE 1 = 1
                   AND tm.T_MEDICAL_RESULT_ID = mer.T_MEDICAL_RESULT_ID
                   AND tm.BRIEF = p_result_brief
                   AND mer.ASSURED_MEDICAL_EXAM_ID = p_assured_medical_exam_id)
    LOOP
      RESULT := cur.note;
      EXIT;
    END LOOP;
