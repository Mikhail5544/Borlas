CREATE OR REPLACE PACKAGE pkg_codegen IS

  -- Author  : ARTUR.BAYTIN, PAVEL.KAPLYA
  ...
  */
  ...
  TYPE t_column IS RECORD(
    ...
    ,nullable       user_tab_cols.nullable%TYPE
  ...
  TYPE t_unique_indexes_list IS TABLE OF user_indexes.index_name%TYPE;
  ...
  */
  ...
  (
    par_table_name user_tab_cols.table_name%TYPE
   ,par_col_name   user_tab_cols.column_name%TYPE
  ) RETURN VARCHAR2;
  ...
  */
  FUNCTION get_cons_search_condition(par_constraint_name user_constraints.constraint_name%TYPE)
    RETURN VARCHAR2;

  FUNCTION gen_package_for_table(par_table_name user_tab_cols.table_name%TYPE) RETURN CLOB;

  /*
    Капля П.
    Процедура перекомпиляции DML пакета для таблицы
  */
  PROCEDURE generate_and_compile(par_table_name user_tables.table_name%TYPE);
...
END pkg_codegen;
/
CREATE OR REPLACE PACKAGE BODY pkg_codegen IS

  gc_use_entity    BOOLEAN := TRUE;
  gc_use_doc_templ BOOLEAN := TRUE;

  gc_doc_templ_exists BOOLEAN;
  ...
  SUBTYPE t_tag_name IS VARCHAR2(50);
  ...
  gc_package_prefix           CONSTANT VARCHAR2(4) := 'dml_';
  ...
  gv_primary_key_column user_tab_cols.column_name%TYPE;
  gv_table_name         user_tab_cols.table_name%TYPE;
  ...
  */
  ...
  FUNCTION check_entity_exists RETURN BOOLEAN IS
    v_entity_exists NUMBER(1);
  BEGIN
    SELECT COUNT(*)
      INTO v_entity_exists
      FROM dual
     WHERE EXISTS (SELECT NULL FROM entity e WHERE e.brief = gv_table_name);
    RETURN v_entity_exists = 1;
  END check_entity_exists;

  FUNCTION check_doc_templ_exists RETURN BOOLEAN IS
    v_exists NUMBER;
  BEGIN
    SELECT COUNT(*)
      INTO v_exists
      FROM dual
     WHERE EXISTS (SELECT NULL FROM TABLE(get_tables) t WHERE t.table_name = 'DOCUMENT')
       AND (SELECT COUNT(*)
              FROM doc_templ dt
                  ,entity    e
             WHERE e.brief = upper(gv_table_name)
               AND e.ent_id = dt.doc_ent_id) = 1;
    RETURN v_exists = 1;
  END check_doc_templ_exists;
  ...
  */
  PROCEDURE init(par_table_name user_tab_cols.table_name%TYPE) IS
    PROCEDURE check_table_exists(par_table_name user_tables.table_name%TYPE) IS
      v_exists NUMBER(1);
    BEGIN
      SELECT COUNT(1)
        INTO v_exists
        FROM dual
       WHERE EXISTS (SELECT NULL FROM user_tables t WHERE t.table_name = upper(par_table_name));
    
      IF v_exists = 0
      THEN
        ex.raise('Таблица ' || par_table_name || ' не существует'
                ,ex.c_no_data_found);
      END IF;
    END check_table_exists;
  BEGIN
    -- Проверка входного параметра
    assert_deprecated(par_table_name IS NULL
          ,'Название таблицы не может быть пустым');
  
    check_table_exists(par_table_name => par_table_name);
  
    gv_table_name := upper(par_table_name);
  
    SELECT t.table_name
          ,LEVEL AS parentness_level BULK COLLECT
      INTO gv_table_list
      FROM user_tables t
     START WITH t.table_name = gv_table_name
    CONNECT BY PRIOR (SELECT uc2.table_name
                        FROM user_constraints  uc_pk
                       ...
                       WHERE uc_pk.table_name = t.table_name
                         ...
                         AND uc_fk.r_constraint_name = uc2.constraint_name) = t.table_name;
    ...
    checks AS
     (SELECT /*+materialize*/
       t.table_name
      ,t.column_name
      ,pkg_codegen.get_cons_search_condition(uc.constraint_name) search_condition
    ...
    SELECT ut.table_name  AS table_name
          ...
          ,CASE
           ...
           END AS nullable
           --,ut.nullable
          ,get_default_col_value(ut.table_name, ut.column_name)
          ,CASE
    ...
    EXCEPTION
      WHEN no_data_found THEN
        ex.raise('Не найдено поле с ограничением "Первичный ключ". Для корректной инициализации, добавьте его в таблицу.');
    END;
  
    SELECT t2.index_name BULK COLLECT
     ...
     WHERE t2.table_name = t.table_name
       ...
       AND NOT EXISTS (SELECT NULL
          ...
          -- Исключить из уникальных индексов поля-объекты. пример AG_PERFOMED_WORK_ACT
       AND NOT EXISTS (SELECT NULL
    ...
    gc_doc_templ_exists := check_doc_templ_exists;
  ...
  */
  ...
  BEGIN
    assert_deprecated(gv_column_list IS NULL
          ,'Список таблиц не инициализирован, выполните init!');
  ...
  */
  ...
  BEGIN
    assert_deprecated(gv_column_list IS NULL
          ,'Список полей не инициализирован, выполните init!');
    FOR v_idx IN gv_column_list.first .. gv_column_list.last
    LOOP
      PIPE ROW(gv_column_list(v_idx));
    END LOOP;
  ...
  FUNCTION get_doc_templ_id RETURN NUMBER IS
  ...
  BEGIN
    SELECT dt.doc_templ_id
      ...
      FROM doc_templ dt
          ,entity    e
     WHERE e.brief = upper(gv_table_name)
       AND e.ent_id = dt.doc_ent_id;
    RETURN v_doc_templ_id;
  END get_doc_templ_id;

  FUNCTION get_doc_templ_brief RETURN doc_templ.brief%TYPE IS
  ...
  BEGIN
    SELECT dt.brief
     ...
     WHERE e.brief = upper(gv_table_name)
       AND e.ent_id = dt.doc_ent_id;
    RETURN v_doc_templ_brief;
  END get_doc_templ_brief;
  ...
  (
   ...
   ,par_max_length    INTEGER
    ...
    c_separator CONSTANT VARCHAR2(2) := '_';
  ...
  BEGIN
    ...
    SELECT NAME
          ...
          ,MAX(nvl(length(NAME), 0)) over() AS max_part_length BULK COLLECT
      INTO v_parts
  ...
  FUNCTION get_table_parent_level(par_table_name user_tables.table_name%TYPE) RETURN INTEGER IS
    v_parent_level INTEGER;
  BEGIN
    SELECT parentness_level
      INTO v_parent_level
      FROM TABLE(get_tables)
     WHERE upper(table_name) = upper(par_table_name);
  ...
  BEGIN
    ...
    par_string := REPLACE(par_string
                         ,'<#TABLE_COMMENT>'
                         , REPLACE(ex.get_table_comment('INS', par_table_name), q'{'}', q'{''}'));
  ...
  BEGIN
    ...
    par_string := REPLACE(par_string
                         ,'<#COLUMN_COMMENT>'
                         , REPLACE(ex.get_column_comment('INS', par_table_name, par_column_name)
                                  , q'{'}'
                                 ,q'{''}'));
  ...
  BEGIN
    SELECT lower(t.table_name)
          ,lower(t.column_name)
          ,MAX(length(t.column_name)) over() BULK COLLECT
      INTO v_column_list
  ...
  (
   ...
   ,par_table_name             user_tab_cols.table_name%TYPE DEFAULT gv_table_name
  ) RETURN t_parameter IS
    v_parameter_str t_parameter;
  
    v_shorten_column_name t_object_name;
  BEGIN
    -- Проверка входных параметров
    assert_deprecated(par_io_type NOT IN (gc_in, gc_out)
          ,'Тип передачи "' || par_io_type || '" значения параметра не поддерживается');
    ...
    ELSIF gc_use_doc_templ
          AND gc_doc_templ_exists
          AND par_table_name = 'DOCUMENT'
          AND par_column_name = 'DOC_TEMPL_ID'
    THEN
      v_parameter_str := v_parameter_str || ' DEFAULT ' || get_doc_templ_id;
  ...
  */
  ...
  BEGIN
    assert_deprecated(par_ddl_operation NOT IN (gc_insert, gc_update, gc_delete, gc_insert_noout)
          ,'Операция "' || nvl(par_ddl_operation, 'NULL') || '" не поддерживается');
                        ...
                        FROM (SELECT cl.table_name
                                    ...
                                    ,CASE
                                       ...
                                       ELSE
                                        gc_in
                                     END AS io_type
                                FROM TABLE(pkg_codegen.get_columns) cl
                                    ,TABLE(get_tables) t
  ...
  BEGIN
      ...
      */
      ...
      FOR rec IN (WITH checks AS
                    ...
                    SELECT ut.table_name  AS table_name
                          ...
                          ,CASE
                           ...
                           END AS nullable
                           --,ut.nullable
                          ,get_default_col_value(ut.table_name, ut.column_name) data_default
                          ,ut.column_id
      ...
      LOOP
        ...
        v_declaration := v_declaration || rpad(lower(rec.column_name), rec.max_length, ' ') ||
                         lower(gv_current_schema || '.' || rec.table_name || '.' || rec.column_name) ||
                         '%TYPE';
    ...
    ELSE
      ...
      LOOP
        ...
        ELSIF gc_use_doc_templ
              AND gc_doc_templ_exists
              AND rec.table_name = 'DOCUMENT'
              AND rec.column_name = 'DOC_TEMPL_ID'
        THEN
          v_declaration := v_declaration || ' DEFAULT ' || get_doc_templ_id;
        END IF;
  ...
  */
  ...
  BEGIN
    assert_deprecated(par_ddl_operation NOT IN (gc_insert
                                    ,gc_update
                                    ,gc_delete
                                    ,gc_insert_noout
                                    ,gc_insert_record
  ...
  BEGIN
	  ...
	  THEN' || chr(10) || gen_select_record || 't' ||
                       get_table_parent_level(gv_table_name) || '.<#PK_FIELD_NAME> = ' ||
                       get_shorten_parameter(gv_primary_key_column) || '
			 FOR UPDATE NOWAIT;
  ...
  BEGIN
    ...
    assert_deprecated(' || get_shorten_parameter(gv_primary_key_column) ||
                       ' IS NULL, ''Запись для блокировки должна быть указана'');';
    ...
    v_function_body := v_function_body || chr(10) || q'{
  EXCEPTION
    WHEN pkg_oracle_exceptions.resource_busy_nowait THEN
      ex.raise('Не удалось заблокировать запись в таблице "<#TABLE_COMMENT>"');
    WHEN no_data_found THEN
      ex.raise('Не удалось найти запись для блокировки в таблице "<#TABLE_COMMENT>"');
    WHEN OTHERS THEN
      ex.raise;
  END lock_record}';
  ...
  (';
    ELSE
      v_function_text := chr(10) || '  FUNCTION get_id_by_' ||
                         get_shorten_variable_name(get_aggregated_column_list(v_column_list)
                                                  ,gc_object_name_length - length('get_id_by_')) || '
  (';
    END IF;
  ...
  BEGIN
    ...
    IF v_column_list.count = 1
    THEN
      v_func_name := 'get_rec_by_' ||
                     get_shorten_variable_name(get_aggregated_column_list(v_column_list)
                                              ,gc_object_name_length - length('get_rec_by_'));
    ELSE
      v_func_name := 'rec_by_' ||
                     get_shorten_variable_name(get_aggregated_column_list(v_column_list)
  ...
  (';
    ELSE
      v_function_text := chr(10) || '  FUNCTION get_rec_by_' ||
                         get_shorten_variable_name(get_aggregated_column_list(v_column_list)
                                                  ,gc_object_name_length - length('get_rec_by_')) || '
  (';
    END IF;
...
' || gen_select_record;
                  ...
                  ELSE
                   CASE gv_table_list.count
                     ...
                     ELSE
                      chr(10) || '        '
                   END || ' AND '
                END || 't' || get_table_parent_level(v_column_list(i).table_name) || '.' || v_column_list(i)
    ...
    THEN
      v_body := v_body || chr(10) ||
                '          ex.raise(''Не найдена запись в таблице "<#TABLE_COMMENT>" по значению поля ';
    ELSE
      v_body := v_body || chr(10) ||
                '          ex.raise(''Не найдена запись в таблице "<#TABLE_COMMENT>" по значению полей ';
    END IF;
    FOR i IN v_column_list.first .. v_column_list.last
    LOOP
      v_column_comment := ex.get_column_comment('INS'
                                               ,v_column_list(i).table_name
                                               ,v_column_list(i).column_name);
    
      v_body := v_body || '"' || v_column_comment || '"';
      IF i < v_column_list.count
      THEN
        v_body := v_body || ', ';
      END IF;
    END LOOP;
  ...
  BEGIN
    BEGIN
' || gen_select_record || 't' || get_table_parent_level(gv_table_name) ||
    ...
    EXCEPTION
      WHEN no_data_found THEN
        IF par_raise_on_error
        THEN
          ex.raise('Не найдена запись в таблице "<#TABLE_COMMENT>" по значению поля "ROWID"');
        END IF;
    END;
    RETURN vr_record;
  ...
  */
  FUNCTION gen_insert_body RETURN CLOB IS
    ...
    BEGIN
      ...
      IF v_exists = 0
      THEN
        ex.raise(par_message => 'Последовательность для заполнения первичного ключа не существует'
  ...
  BEGIN
    ...
    LOOP
      ...
      v_header_text := chr(10) || '    INSERT INTO <#TABLE_NAME>' || chr(10);
    
      parse_and_replace(v_header_text, rec_tab.table_name);
      ...
      LOOP
        ...
        ELSE
          ...
          v_param_text := chr(10) || '      ,' || CASE
                            WHEN vr_columns.column_name = 'ENT_ID'
                                 AND ent.id_by_brief(gv_table_name) IS NOT NULL THEN
                             'gc_entity_id'
                            ELSE
                             get_shorten_parameter(vr_columns.column_name)
                          END;
        END IF;
  ...
  */
  ...
  BEGIN
    ...
    append_lob(par_lob  => v_exception_block
              ,par_buff => '  EXCEPTION
    WHEN pkg_oracle_exceptions.bulk_dml THEN
			' || gc_collect_bulk_ex_par_name || ' := TRUE;
      ex.process_bulk_exceptions(' || gc_bulk_ex_list_par_name || ');' ||
                           chr(10));
  ...
  FUNCTION gen_get_entity_id_spec RETURN VARCHAR2 IS
    v_function_text VARCHAR2(4000);
  BEGIN
    v_function_text := '  FUNCTION get_entity_id RETURN entity.ent_id%TYPE';
  
    RETURN v_function_text;
  END gen_get_entity_id_spec;

  FUNCTION gen_get_entity_id_body RETURN VARCHAR2 IS
    v_function_text VARCHAR2(4000);
    v_body          VARCHAR2(4000);
  BEGIN
    v_function_text := gen_get_entity_id_spec;
  ...
  BEGIN
    ...
    RETURN v_function_text || v_body;
  END gen_get_entity_id_body;

  FUNCTION gen_doc_templ_consts RETURN VARCHAR2 IS
    v_function_text VARCHAR2(32767);
  BEGIN
    v_function_text := '  doc_templ_id CONSTANT doc_templ.doc_templ_id%type := ' || get_doc_templ_id || ';
  doc_templ_brief CONSTANT doc_templ.brief%type := ''' || get_doc_templ_brief ||
                       ''';';
    RETURN v_function_text;
  END gen_doc_templ_consts;
  ...
  */
  ...
  BEGIN
    ...
    append_lob(par_lob  => v_specification
              ,par_buff => 'CREATE OR REPLACE PACKAGE ' || gc_package_prefix || lower(gv_table_name) ||
                           gc_package_suffix || ' IS' || chr(10));
    ...
    THEN
      append_lob(par_lob => v_specification, par_buff => chr(10) || gen_get_entity_id_spec);
      append_lob(par_lob => v_specification, par_buff => ';' || chr(10));
    
      IF gc_use_doc_templ
         AND check_doc_templ_exists()
      THEN
        append_lob(par_lob => v_specification, par_buff => chr(10) || gen_doc_templ_consts || chr(10));
      END IF;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    append_lob(par_lob  => v_body
              ,par_buff => 'CREATE OR REPLACE PACKAGE BODY ' || gc_package_prefix ||
    ...
    THEN
      append_lob(par_lob  => v_body
                ,par_buff => chr(10) ||
                             '  gc_entity_id CONSTANT entity.ent_id%TYPE := ent.id_by_brief(''' ||
                             upper(gv_table_name) || ''');' || chr(10) || chr(10));
    
      append_lob(par_lob => v_body, par_buff => gen_get_entity_id_body);
      append_lob(par_lob => v_body, par_buff => ';' || chr(10));
    END IF;
  ...
  FUNCTION gen_package_for_table(par_table_name user_tab_cols.table_name%TYPE) RETURN CLOB IS
    v_specification CLOB;
    v_body          CLOB;
    v_package       CLOB;
  ...
  PROCEDURE generate_and_compile(par_table_name user_tables.table_name%TYPE) IS
    v_pkg_spec CLOB;
    v_pkg_body CLOB;
    c1         INTEGER;
    c2         INTEGER;
  ...
  BEGIN
    ...
    LOOP
      generate_and_compile(par_table_name => rec.table_name);
    END LOOP;
