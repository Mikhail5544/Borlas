CREATE OR REPLACE PACKAGE pkg_rep_utils_ins11_test IS
...
END pkg_rep_utils_ins11_test;
/
CREATE OR REPLACE PACKAGE BODY pkg_rep_utils_ins11_test IS
  ...
  BEGIN
    SELECT COUNT(agch.agent_id)
      INTO col
      FROM ins.ven_ag_contract_header agch
      JOIN (SELECT * FROM TABLE(pkg_rep_utils_ins11_test.get_ag_contract_id(st, cat))) agch_id
  ...
  BEGIN
    ...
    IF month_start IS NOT NULL
    THEN
      SELECT nvl(SUM(ppag.part_agent / 100 * pp.premium), 0)
        INTO sales
        FROM ins.ven_ag_contract_header agch
        JOIN ins.ven_ag_contract agc
          ON agc.ag_contract_id = agch.last_ver_id
  ...
  BEGIN
    ...
    SELECT COUNT(*)
      ...
      JOIN ins.ven_ag_contract agc
        ON agc.ag_contract_id = agch.last_ver_id
      JOIN (SELECT * FROM TABLE(pkg_rep_utils_ins11_test.get_part_time_id)) agch_id
        ON agch.ag_contract_header_id = agch_id.column_value
  ...
  BEGIN
    SELECT nvl(SUM(ppag.part_agent / 100 * pp.premium), 0)
      ...
      JOIN ins.ven_ag_contract agc
        ON agc.ag_contract_id = agch.last_ver_id
      JOIN (SELECT * FROM TABLE(pkg_rep_utils_ins11_test.get_ag_chanal_id(chanal))) agch_id
        ON agch.ag_contract_header_id = agch_id.column_value
      LEFT JOIN ins.ven_p_policy_agent ppag
        ON ppag.ag_contract_header_id = agch.ag_contract_header_id
      LEFT JOIN ins.ven_p_pol_header pph
  ...
  BEGIN
    SELECT nvl(SUM(ppag.part_agent / 100 * pp.premium), 0)
      ...
      JOIN ins.ven_ag_contract agc
        ON agc.ag_contract_id = agch.last_ver_id
      JOIN (SELECT * FROM TABLE(pkg_rep_utils_ins11_test.get_ag_other_chanal_id)) agch_id
        ON agch.ag_contract_header_id = agch_id.column_value
      LEFT JOIN ins.ven_p_policy_agent ppag
        ON ppag.ag_contract_header_id = agch.ag_contract_header_id
      LEFT JOIN ins.ven_p_pol_header pph
  ...
  (
   ...
   ,YEAR  VARCHAR2
    ...
    sal NUMBER;
  BEGIN
    SELECT nvl(SUM(pa.part_agent / 100 * pp.premium), 0)
      INTO sal
      FROM ins.ven_p_pol_header ph
  ...
  BEGIN
    SELECT COUNT(*)
      INTO num
      FROM ins.ven_p_pol_header ph
      JOIN (SELECT * FROM TABLE(pkg_rep_utils_ins11_test.get_pol_dep_ch_id(depart_id, chanal))) tbl
        ON ph.policy_header_id = tbl.column_value
     WHERE ins.pkg_rep_utils.get_notice_date(ph.policy_header_id) BETWEEN dleft AND dright;
    RETURN(num);
  EXCEPTION
    WHEN OTHERS THEN
      RETURN 0;
  ...
  BEGIN
    FOR rec IN (SELECT NAME
                      ,ag_id
                      ,ch_brief
                  FROM TABLE(pkg_rep_utils_ins11_test.get_list_departments_chanals(dright)))
    LOOP
  ...
  BEGIN
    SELECT COUNT(*)
      INTO num
      FROM ins.ven_c_claim_header clh
      JOIN (SELECT * FROM TABLE(pkg_rep_utils_ins11_test.get_claim_header_id(br, rtype))) clh_id
  ...
  BEGIN
    ...
    FOR t_rec IN (SELECT DISTINCT nvl(tbl.agent_id, -1)
                    ...
                    FROM (SELECT DISTINCT (SELECT tt.ag_id
                                             FROM ins_dwh.rep_sr_tmp tt
                                            WHERE tt.pol_id = ph.policy_header_id) AS agent_id
                                         ,sch.brief AS ch_br
                                         ,sch.description AS ch_name
                                         ,ph.agency_id AS dep_id
                                         ,pkg_rep_utils_ins11_test.get_pol_agency_name(ph.agency_id) AS dep_name
                                         ,pr.brief AS pr_br
  ...
  */
  ...
  BEGIN
    SELECT nvl(SUM(pp.premium), 0)
      ...
      JOIN ins.ven_p_policy pp
        ON pp.policy_id = ph.policy_id
      JOIN (SELECT *
              FROM TABLE(pkg_rep_utils_ins11_test.get_policy_id(dleft
                                                               ,dright
                                                               ,ag_id
                                                               ,dep_id
                                                               ,ch_br
  ...
  BEGIN
    SELECT nvl(SUM(pp.premium), 0)
      ...
      JOIN ins.ven_p_policy pp
        ON pp.policy_id = ph.policy_id
      JOIN (SELECT *
              FROM TABLE(pkg_rep_utils_ins11_test.get_pol_dec_anderr_id(dleft
                                                                       ,dright
                                                                       ,ag_id
                                                                       ,dep_id
                                                                       ,ch_br
  ...
  BEGIN
    SELECT nvl(SUM(pp.premium), 0)
      ...
      JOIN ins.ven_p_policy pp
        ON pp.policy_id = ph.policy_id
      JOIN (SELECT *
              FROM TABLE(pkg_rep_utils_ins11_test.get_pol_dec_other_id(dleft
                                                                      ,dright
                                                                      ,ag_id
                                                                      ,dep_id
                                                                      ,ch_br
  ...
  BEGIN
    SELECT nvl(SUM(pp.premium), 0)
      ...
      JOIN ins.ven_p_policy pp
        ON pp.policy_id = ph.policy_id
      JOIN (SELECT *
              FROM TABLE(pkg_rep_utils_ins11_test.get_pol_active_id(dleft
                                                                   ,dright
                                                                   ,ag_id
                                                                   ,dep_id
                                                                   ,ch_br
  ...
  BEGIN
    -- чистим таблицу 
    DELETE ins_dwh.rep_sr_tmp;
    -- заполняем таблицу
    FOR rec IN (SELECT ph.policy_header_id AS pol_id
                      ,pkg_rep_utils_ins11_test.get_agch_id_by_polid(ph.policy_header_id) AS ag_id
                  FROM ins.ven_p_pol_header ph
                 WHERE ph.start_date BETWEEN dleft AND dright)
    LOOP
      INSERT INTO ins_dwh.rep_sr_tmp tt (tt.pol_id, tt.ag_id) VALUES (rec.pol_id, nvl(rec.ag_id, -1));
    END LOOP;
    COMMIT;
  END;
  ...
  BEGIN
    ...
    FOR rec IN (SELECT ag_id
                      ...
                      ,pr_br
                      ,pr_name
                  FROM TABLE(pkg_rep_utils_ins11_test.get_tbl_ag_ch_pr(dleft, dright)))
    ...
    LOOP
      ...
      SELECT COUNT(*)
        INTO act_num_ytd
        FROM TABLE(pkg_rep_utils_ins11_test.get_pol_active_id(first_day
                                                             ,dright
                                                             ,rec.ag_id
                                                             ,rec.dep_id
                                                             ,rec.ch_br
  ...
  BEGIN
    -- заполняем таблицу соответствий "агенты - договора"
    fill_tbl_pol_ag(dleft, dright);
    -- вычисляем первую дату года
    first_day := trunc(dleft, 'yyyy');
    --first_day:= to_date ('01.01.' || to_char(dleft,'yyyy'),'dd.mm.yyyy');
    ...
    FOR rec IN (SELECT ag_id
                      ...
                      ,pr_br
                      ,pr_name
                  FROM TABLE(pkg_rep_utils_ins11.get_tbl_ag_ch_progr(dleft, dright)))
    ...
    LOOP
      ...
      -- заключенные договора
      pol_ape        := to_number(get_ape_policy_pr(dleft
                                                   ,dright
                                                   ,rec.ag_id
                                                   ,rec.dep_id
                                                   ,rec.ch_br
                                                   ,rec.pr_br)); -- годовая премия
      pol_pay_amount := get_pol_pay_amount_pr(dleft
                                             ,dright
                                             ,rec.ag_id
                                             ,rec.dep_id
                                             ,rec.ch_br
      ...
      -- договора заключенные и расторгнутые (заявление страхователя, отказ андеррайтинга)
      dec_ape        := to_number(get_ape_pol_dec_pr(dleft
                                                    ,dright
                                                    ,rec.ag_id
                                                    ,rec.dep_id
                                                    ,rec.ch_br
                                                    ,rec.pr_br)); -- годовая премия
      dec_pay_amount := get_pol_dec_amount_pr(dleft
                                             ,dright
                                             ,rec.ag_id
                                             ,rec.dep_id
                                             ,rec.ch_br
      ...
      --активные договора
      act_ape        := to_number(get_ape_pol_act_pr(dleft
                                                    ,dright
                                                    ,rec.ag_id
                                                    ,rec.dep_id
                                                    ,rec.ch_br
                                                    ,rec.pr_br)); -- годовая премия
      act_pay_amount := get_pol_pay_amount_act_pr(dleft
                                                 ,dright
                                                 ,rec.ag_id
                                                 ,rec.dep_id
                                                 ,rec.ch_br
      ...
      -- активные договора с накопленным итогом с начала года
      act_ape_ytd        := to_number(get_ape_pol_act_pr(first_day
                                                        ,dright
                                                        ,rec.ag_id
                                                        ,rec.dep_id
                                                        ,rec.ch_br
                                                        ,rec.pr_br)); -- годовая премия
      act_pay_amount_ytd := get_pol_pay_amount_act_pr(first_day
                                                     ,dright
                                                     ,rec.ag_id
                                                     ,rec.dep_id
                                                     ,rec.ch_br
      ...
      SELECT COUNT(*)
        INTO act_num_ytd
        FROM TABLE(pkg_rep_utils_ins11.get_pol_active_id_pr(first_day
                                                           ,dright
                                                           ,rec.ag_id
                                                           ,rec.dep_id
                                                           ,rec.ch_br
      ...
      -- договора заключенные и расторгнутые (по иным причинам)
      dec_oth_ape        := to_number(get_ape_pol_dec_oth_pr(dleft
                                                            ,dright
                                                            ,rec.ag_id
                                                            ,rec.dep_id
                                                            ,rec.ch_br
                                                            ,rec.pr_br)); -- годовая премия
      dec_oth_pay_amount := get_pol_dec_amount_oth_pr(dleft
                                                     ,dright
                                                     ,rec.ag_id
                                                     ,rec.dep_id
                                                     ,rec.ch_br
      ...
      insert_row_to_sr_prog(rec.ch_name, rec.dep_name, rec.ag_fio, rec.pr_name, 'APE (net)', act_ape);
      insert_row_to_sr_prog(rec.ch_name
                           ,rec.dep_name
                           ,rec.ag_fio
                           ,rec.pr_name
