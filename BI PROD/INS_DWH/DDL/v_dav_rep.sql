CREATE OR REPLACE FORCE VIEW INS_DWH.V_DAV_REP AS
SELECT --arh.num ved,
     --  ar.ag_roll_id,
     ...
     --  appad.*,
       ...
       (SELECT DISTINCT LAST_VALUE(ach1.num)
          ...
          FROM ins.ven_ag_contract_header ach1
         WHERE ach1.agent_id = ach.agent_id) first_ad_num,
       (SELECT MIN(ach1.date_begin)
          FROM ins.ag_contract_header ach1
         WHERE ach1.agent_id = ach.agent_id) first_ad_begin,
       (SELECT dsr.name FROM ins.doc_status_ref dsr WHERE dsr.doc_status_ref_id= apw.status_id) AD_status_CT,
       ins.doc.get_doc_status_name(ac.ag_contract_id) AD_status_AP,
       ...
       ins.ent.obj_name('CONTACT',ins.pkg_policy.get_policy_contact(pp.policy_id,'Страхователь')) insuer,
       (SELECT dsr.name FROM ins.doc_status_ref dsr WHERE dsr.doc_status_ref_id= apdp.policy_status_id) Active_version_status_CT,
       ins.doc.get_doc_status_name(apdp.policy_id) Active_CT_version_status_AP,
       ins.doc.get_doc_status_name(ph.policy_id) Active_version_status_AP,
       f.name,
       epg.num EPG,
       epg.reg_date EPG_DATE,
       (SELECT dsr.name FROM ins.doc_status_ref dsr WHERE dsr.doc_status_ref_id = appad.epg_status_id) EPG_status_CT,
      ...
      -- apdp.check_3,
       ...
       round(ins.pkg_tariff_calc.calc_coeff_val('SGP_plan_for_DAV',ins.t_number_type(TO_CHAR(arh.date_end,'YYYYMMDD'),apd.mounth_num, apw.sgp1))
       *(arh.date_end + 1-(SELECT MIN(ach1.date_begin)
                          FROM ins.ag_contract_header ach1
                         WHERE ach1.agent_id = ach.agent_id))
       /(arh.date_end + 1 - arh.date_begin))
       ELSE
       ins.pkg_tariff_calc.calc_coeff_val('SGP_plan_for_DAV', ins.t_number_type(TO_CHAR(arh.date_end,'YYYYMMDD'),apd.mounth_num, apw.sgp1)) END sgp_plan,
  ...
  FROM ins.ven_ag_roll_header arh,
       ...
       ins.ag_perf_work_pay_doc appad,
       ins.p_policy pp,
       ins.p_pol_header ph,
       ins.ven_ag_contract_header ach,
       ins.ag_contract ac,
       ins.ven_ac_payment epg,
       ins.ven_ac_payment pay,
       ins.doc_set_off dso_p,
       ins.t_product tp,
     --  ag_stat_agent asa,
       ins.ag_stat_hist ash,
       ins.T_CONTACT_TYPE tct,
       ins.t_payment_terms tpt,
       ins.fund f
 WHERE 1=1
   and (select CLIENT_IDENTIFIER from v$session where audsid = userenv('SESSIONID')) in ('YPLATOVA','TKIM','JKOVRIGINA','EGORNOVA','TEST_ANY_ROLE')
 ...
 --'000151'
   ...
   AND ins.Pkg_Agent_1.get_status_by_date(ach.ag_contract_header_id, SYSDATE) = ac.ag_contract_id
