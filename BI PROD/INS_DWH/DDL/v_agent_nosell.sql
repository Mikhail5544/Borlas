CREATE OR REPLACE FORCE VIEW INS_DWH.V_AGENT_NOSELL AS
select agent_name, sales_ch, department, leader_name, date_begin, start_date, round(sysdate - start_date,0) days
from (
select agent_name, sales_ch, department, leader_name, date_begin, max(start_date) start_date
from (
...
group by agent_name, sales_ch, department, leader_name, date_begin
)
where sysdate - start_date > (SELECT r.param_value
