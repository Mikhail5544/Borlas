create materialized view INS_DWH.FC_RE_COVER
...
select nvl(dm_ph.policy_header_id, -1) policy_header_id,
       ...
       mc.reinsurer_id reinsurer_contact_id,
      ...
      ,
       case when rc.INS_AMOUNT = 0 then 0
      else
       round((rc.PART_SUM/rc.INS_AMOUNT)*100,2)
       end part_percent
      ,
      ...
      ,
       0 charge_repremium_rsbu_rur --начисленная премия пер-ка РБСУ
      ,
       0 charge_commission_rsbu_rur--начисленная комиссия пер-ка РБСУ
      ,
       0 charge_repremium_ifrs_rur --начисленная премия пер-ка МСФО
      ,
       0 charge_commission_ifrs_rur --начисленная комиссия пер-ка МСФО
      ,
       0 charge_rate -- курс на дату начисления
      ,
       0 pay_rate -- курс на дату выплаты
      ,
