...
select pr.description product_name, -- продукт
       pp.pol_ser, -- серия
       pp.pol_num, -- номер
       c.CONTACT_ID issuer_id, -- ИД клиента
       ins.ent.obj_name(c.ent_id,c.contact_id) issuer, -- страхователь - Клиент
       f1.brief fund, -- валюта отв
       f2.brief fynd_pay, -- валюта расчетов
       pp.premium premium_amount, -- премия
       ins.pkg_rep_utils.pay_prem_policy(pp.POLICY_ID) premium_pay_amount, --получено премии
       pay.pay_summ damage_pay_amount, --Выплаченные убытки
       decode (nvl(decl.dec_summ,0) - nvl(decline.decline_summ,0),0,0,
          decode ((nvl(decl.dec_summ,0) - nvl(decline.decline_summ,0))/
                    ...
                    0)) damage_declare_amount --Заявленные положит убытки
