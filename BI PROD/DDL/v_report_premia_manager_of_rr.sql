CREATE OR REPLACE FORCE VIEW V_REPORT_PREMIA_MANAGER_OF_RR AS
SELECT count(1) over() row_cnt,
agd.AGENT_REPORT_ID,
 pp.pol_ser || '-' || pp.pol_num pol_num, pi.contact_name strahov_name,
PKG_AGENT_1.get_agent_status_name_by_date(agch.ag_contract_header_id,pp.START_DATE) status_mng,
       ...
       vca.category_name status_name,
         GetManagerByDate(ava.date_end,c2.ag_contract_header_id) as strahov_name_mng,--правил Колганов 12.12.2007
...
FROM ven_p_policy pp
  ...
  WHERE
  agd.agent_report_id = PKG_REP_UTILS2.iGetVal('P_AG_REP_ID') and
