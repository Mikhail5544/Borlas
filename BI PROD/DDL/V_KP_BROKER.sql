CREATE OR REPLACE VIEW V_KP_BROKER AS
SELECT agh.num,
       ...
       (SELECT rf.name
        ...
        (SELECT tel.telephone_number FROM ins.cn_contact_telephone tel
         WHERE tel.contact_id = cpol.contact_id
           AND tel.telephone_type = 1
           AND tel.status = 1
           AND ROWNUM = 1) tel1,--"Личный"
        (SELECT tel.telephone_number FROM ins.cn_contact_telephone tel
         WHERE tel.contact_id = cpol.contact_id
           AND tel.telephone_type = 2
           AND tel.status = 1
           AND ROWNUM = 1) tel2,--"Рабочий"
        (SELECT tel.telephone_number FROM ins.cn_contact_telephone tel
         WHERE tel.contact_id = cpol.contact_id
           AND tel.telephone_type = 3
           AND tel.status = 1
           AND ROWNUM = 1) tel3,--"Домашний"
        (SELECT tel.telephone_number FROM ins.cn_contact_telephone tel
         WHERE tel.contact_id = cpol.contact_id
           AND tel.telephone_type = 4
           AND tel.status = 1
           AND ROWNUM = 1) tel4,--"Мобильный"
        (SELECT tel.telephone_number FROM ins.cn_contact_telephone tel
         WHERE tel.contact_id = cpol.contact_id
           AND tel.telephone_type = 181
           AND tel.status = 1
           AND ROWNUM = 1) tel5,--"Контактный"
        ...
        WHERE d.document_id = a.payment_id AND
         ...
         NVL(ch.description,'-') sales_chnl,
         NVL(aghr.num,'-') num_recruit,
         NVL(cr.obj_name_orig,'-') name_recruit
...
GRANT SELECT ON V_KP_BROKER TO INS_EUL;
GRANT SELECT ON V_KP_BROKER TO INS_READ;
