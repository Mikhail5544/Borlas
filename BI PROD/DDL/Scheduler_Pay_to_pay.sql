begin
  sys.dbms_scheduler.disable(name => 'INS.PAY_TO_PAY');
  sys.dbms_scheduler.set_attribute(name => 'INS.PAY_TO_PAY', attribute => 'job_action', value => 'DECLARE
  v_count PLS_INTEGER;
  ...
  BEGIN
    ...
    FOR r IN (
      ...
      WHERE ph.policy_header_id = pp.pol_header_id
         ...
         AND (doc.get_doc_status_brief(ph.policy_id) not in (''BREAK'',
                                                            ''CANCEL'',
                                                            ''READY_TO_CANCEL'',
                                                            ''STOPED'',
                                                            ''STOP'',
                                                            ...
                                                            ''QUIT_REQ_QUERY'',
                                                            ''QUIT_TO_PAY'')
         /*AND*/ OR pkg_policy.get_last_version_status(ph.policy_header_id) NOT IN (''Готовится к расторжению'',
                                                                             ''Завершен'',
                                                                             ''Отменен'',
                                                                             ''Приостановлен'',
                                                                             ''Расторгнут'',
  ...
  END;');
  sys.dbms_scheduler.set_attribute(name => 'INS.PAY_TO_PAY', attribute => 'start_date', value => to_date('11-07-2009 04:40:02', 'dd-mm-yyyy hh24:mi:ss'));
  sys.dbms_scheduler.enable(name => 'INS.PAY_TO_PAY');
end;
/
