        ...
        NVL(
            (SELECT MAX(agd.doc_date)
             ...
             WHERE agd.ag_doc_type_id = adt.ag_doc_type_id
               ...
               AND doc.get_doc_status_brief(agd.ag_documents_id) NOT IN ('PROJECT','CANCEL')
               AND agd.ag_contract_header_id = ag.ag_contract_header_id),
            ag.date_begin
         ) DOC_DATE_PAY_PROP
from ins.ag_contract_header ag,
     ...
     (select nvl(ii.is_default,0) is_default, ii.contact_id, ii.serial_nr, ii.id_value, ii.issue_date, ii.place_of_issue
      ...
      (select bank.ag_contract_header_id, bik.id_value bik_bank, bik.obj_name_orig name_bank, bank.account raccount_bank, REPLACE(REPLACE(bank.payment_props,'-'),'X') licaccount_bank
      from ins.ven_ag_bank_props bank,
