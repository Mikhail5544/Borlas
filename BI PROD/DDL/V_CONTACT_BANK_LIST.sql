CREATE OR REPLACE FORCE VIEW V_CONTACT_BANK_LIST AS
  ...
  FROM  contact c,
        ...
        ( SELECT  cca.contact_id, 
                  DECODE( ca.province_name, 'Москва', 0,
                    'Санкт-Петербург', ( -1 ), 
                    0/*( SELECT ca.city_id FROM t_city ci WHERE ca.city_id = ci.city_id )*/ ) city_id, 
                  DECODE( ca.province_name, 'Москва', 'Москва', 
                    'Санкт-Петербург', 'Санкт-Петербург', 
                    ca.city_name/*( SELECT ca.city_name FROM t_city ci WHERE ca.city_id = ci.city_id )*/ ) city_name
            FROM  cn_contact_address cca,
                  cn_address         ca
            WHERE cca.is_default = 1
