CREATE OR REPLACE VIEW V_BSO_CONTACT AS
...
from bso bb,
     bso_series ser,
     bso_type tp,
     (select h.bso_id, max(h.num) max_number
      from bso_hist h,
           department dep
      where h.hist_date <= (SELECT r.param_value
                          FROM ins_dwh.rep_param r
                          WHERE r.rep_name = 'bso_contact'
...
where bb.bso_series_id = ser.bso_series_id
      ...
      and h.num = mh.max_number
      and h.bso_id = mh.bso_id
      ...
      --and bb.num like '%028301%' and ser.series_name = '130'
      and h.hist_date <= (SELECT r.param_value
                          FROM ins_dwh.rep_param r
                          WHERE r.rep_name = 'bso_contact'
                            AND r.param_name = 'date_from')
      and dep.name IN (SELECT r.param_value
