create or replace force view v_as_property_spec_mach as
select p.as_property_id,
       ...
       p.deduct_value,
       p.t_moveable_machines_id,
       (select name from ven_t_moveable_machines where t_moveable_machines_id = p.t_moveable_machines_id) moveable_machines,
       (select f.brief
