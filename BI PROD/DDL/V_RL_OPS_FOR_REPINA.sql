CREATE OR REPLACE VIEW V_RL_OPS_FOR_REPINA AS
WITH all_agent AS
              ...
              (select ci.contact_id, ci.id_value id_value, row_number() over(partition by ci.contact_id order by nvl(ci.is_default,0)) rn
               ...
               where ci.id_type = it.id
                 ...
                 /*(SELECT param_value
                  ...
                  WHERE rep_name = 'ag_tree'
                    and param_name = 'rep_date')*/ between ci.issue_date and nvl(ci.termination_date,to_date('31.12.3000','dd.mm.yyyy'))
               ) lv
 ...
 (select /*+INDEX(cnp IX_AG_CONTRACT_01) */
   ...
   where pad.ag_contract_header_id = ch.ag_contract_header_id
     ...
     and /*pad.date_begin*//*(select to_date(r.param_value,'dd.mm.yyyy')
                              from ins_dwh.rep_param r
                             where r.rep_name = 'RL_OPS_FOR_REPINA'
                               and r.param_name = 'ag_date')*/
             (SELECT trl.ag_tree_date
             FROM ins.t_rl_ops_date trl
             WHERE trl.enabled = 1) between cn.date_begin and cn.date_end
     ...
     and pad.date_begin            = (select min(pam.date_begin)
                                       ...
                                       where pam.policy_header_id      = pad.policy_header_id
                                         ...
                                         and (SELECT trl.ag_tree_date
                                              FROM ins.t_rl_ops_date trl
                                              WHERE trl.enabled = 1)
                                             /*(select to_date(r.param_value,'dd.mm.yyyy')
                                                from ins_dwh.rep_param r
                                               where r.rep_name = 'RL_OPS_FOR_REPINA'
...
FROM
 ...
 SELECT p.num,
        ...
        /*e.pol_header_id,*/
        (CASE WHEN abs(acp.plan_date - ph.start_date)<5 THEN 1 ELSE 0 END) first_epg,
        /*e.first_epg,*/
        ph.start_date,
        ph.is_group_flag,
        ph.pay_term,
   ...
   FROM ins_dwh.fc_pay_doc         p,
        /*ins_dwh.t_fc_epg           e,*/
        (SELECT aca.num, aca.plan_date, pola.pol_header_id, aca.payment_id
        ...
        -- Заявка №151040
       ,nearest_agent na
  WHERE (CASE WHEN p.pd4_date IS NOT NULL THEN
         p.pd4_date
        ELSE
          p.pay_date
        END) BETWEEN (SELECT trl.rl_from_date
                      FROM ins.t_rl_ops_date trl
                      WHERE trl.enabled = 1)
             AND (SELECT trl.rl_to_date
                  FROM ins.t_rl_ops_date trl
                  WHERE trl.enabled = 1)
  /*nvl(p.pd4_date, p.pay_date) BETWEEN (SELECT trl.rl_from_date
                                             FROM ins.t_rl_ops_date trl
                                             WHERE trl.enabled = 1)
                                    AND (SELECT trl.rl_to_date
                                         FROM ins.t_rl_ops_date trl
...
--               etl.npf_policy_assured_m npa,
         ...
         WHERE 1=1
           ...
           and dc.doc_status_ref_id = dsr.doc_status_ref_id
           and rh.ops_sign_date between (SELECT trl.ops_from_date
                                         FROM ins.t_rl_ops_date trl
                                         WHERE trl.enabled = 1)
                                AND (SELECT trl.ops_to_date
                                     FROM ins.t_rl_ops_date trl
                                     WHERE trl.enabled = 1)
                                /*to_date((SELECT r.param_value
                                    FROM ins_dwh.rep_param r
           ...
           AND nop.sign_date between (SELECT trl.ops_from_date
                                      FROM ins.t_rl_ops_date trl
                                      WHERE trl.enabled = 1)
                                AND (SELECT trl.ops_to_date
                                     FROM ins.t_rl_ops_date trl
