create or replace force view dwg_main as
select NULL "Empty0",
     ...
     --  prod.brief || '-' || pl.description "Risk name",
       ...
       NULL "Empty10",	NULL "Empty11",	NULL "Empty12",	NULL "Empty13",
       cn_pol_holder.obj_name "Policy holder",
       NULL "Empty14",	NULL "Empty15",	NULL "Empty16",	NULL "Empty17",	NULL "Empty18",
       decode(pp.is_group_flag,0,'I',1,'C','I') "Corporate policy type",
       NULL "Empty19",
       nvl(pca.reinsurer,bordero_reinsurer.reinsurer_name) "Reinsured",
       NULL "Empty20",	NULL "Empty21",
       ph.start_date "Date orig",
       NULL "Empty22",	NULL "Empty23",	NULL "Empty24",	NULL "Empty25",	NULL "Empty26",
       NULL "Empty27",	NULL "Empty28",	NULL "Empty29",	NULL "Empty30",	NULL "Empty31",
       NULL "Empty32",	NULL "Empty33",	NULL "Empty34",	NULL "Empty35",	NULL "Empty36",
       NULL "Empty37",
       --!!!!!!!! заполнить
       NULL "Year com",
       NULL "Empty38",	NULL "Empty39",	NULL "Empty40",	NULL "Empty41",	NULL "Empty42",
       NULL "Empty43",	NULL "Empty44",	NULL "Empty45",
       --!!!!!!!! заполнить
       --pkg_payment.get_charge_pcover_amount_pfa(pc.start_date, pc.end_date, pc.p_cover_id)
       charging.s "Charge Premium",
       NULL "Empty47",	NULL "Empty48",	NULL "Empty49",	NULL "Empty50",	NULL "Empty51",
       NULL "Empty52",	NULL "Empty53",
       --!!!!!!!! заполнить
       case
         when months_between(pp.end_date + 1, ph.start_date) > 12 then
          case
           ...
           "Risk type gaap",
       NULL "Empty54",	NULL "Empty55",	NULL "Empty56",	NULL "Empty57",
       pc.rvb_value            "Loading",
       NULL "Ii rate benefit",
       NULL "Benefit period name",
       NULL "Date start benefit",
       NULL "Date benefit end",	NULL "Benefit period",	NULL "Date guaranteed end",	NULL "Guaranteed period",	NULL "Date end premium",	NULL "Premium period",
       NULL "Empty58",	NULL "Empty59",	NULL "Empty60",	NULL "Empty61",
       NULL "Empty62",	NULL "Empty63",	NULL "Empty64",	NULL "Empty65",
       NULL "Acquisition total",
       /*
       case tpt.is_periodical
       when 1 then add_months(pc.start_date,decode(tpt.number_of_payments,1,12,2,6,4,3,12,1,12)*trunc(months_between(to_date('30.09.2007',
                                                        'DD.MM.YYYY'), pc.start_date)/decode(tpt.number_of_payments,1,12,2,6,4,3,12,1,12),0))
       when 0 then pc.start_date
       end      "Inst start",
       */
       case tpt.is_periodical
       when 1 then get_start_date_pmnt_period(pc.start_date,decode(tpt.number_of_payments,1,12,2,6,4,3,12,1,12),0,trunc(months_between(to_date('30.09.2007',
                                                        'DD.MM.YYYY'), pc.start_date)/decode(tpt.number_of_payments,1,12,2,6,4,3,12,1,12),0))
       when 0 then pc.start_date
       end      "Inst start",
       ...
       case tpt.is_periodical
       when 1 then add_months(pc.start_date,decode(tpt.number_of_payments,1,12,2,6,4,3,12,1,12)*(trunc(months_between(to_date('30.09.2007',
                                                        'DD.MM.YYYY'), pc.start_date)/decode(tpt.number_of_payments,1,12,2,6,4,3,12,1,12),0)+1))-1
  ...
  from p_cover pc,
       ...
       (select policy_id
          from (select max(pp.version_num) over(partition by pp.pol_header_id) m,
                       ...
                       pp.version_num
                  from p_policy pp
                 where doc.get_doc_status_brief(pp.policy_id,
                                                 sysdate
                                                /*to_date('17.10.2007',
                                                        'DD.MM.YYYY')*/) not in
...
--        and ph.policy_header_id = pp.pol_header_id(+)
     ...
     --   and t.trans_date between pc.start_date and pc.end_date
         ...
         (select prod1.description, prod1.product_id from t_product prod1 where   prod1.brief like '%CR%') cr_prod
         ,  (select sum(av1.trans_amount) s, av1.p_cover_id from trans_av_dwh av1
group by av1.p_cover_id) av_trans
,
(select distinct mc.reinsurer_id, rc.p_cover_id, con.obj_name reinsurer_name
from re_cover rc
   , re_main_contract mc, contact con
...
--   and bordero_reinsurer.reinsurer_id = reinsurer_contact.contact_id
  ...
  -- and ll.description = 'Доп. программа №3 Освобождение от уплаты дальнейших взносов'
   --and pay_coef.pcc_val is null
   --and pp.ext_id is null


  -- and charging.trans_date between pc.start_date and pc.end_date

  -- and months_between(pc.end_date + 1, pc.start_date) <= 12
  -- and ig.life_property = 1
  -- order by 2, 1
   --and pp.num like '%16614%'
   --and ph.policy_header_id = 780110

  --and pc.p_cover_id = 332531
  --and prod.description like '%CR%'

  -- and pp.ext_id = 20953
  --and cn_pol_holder.obj_name = 'ТИТОВА ЕВГЕНИЯ ВЛАДИМИРОВНА'
--select * from doc_status_ref

-- 008144 feninav
-- select * from cn_person
;
