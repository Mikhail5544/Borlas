create or replace view V_NEXT_UNPAID_FEE_PREV as
SELECT
/*
Расторжение по неоплате очередного взноса за прошлый месяц
...
DISTINCT (SELECT insp.policy_id
 ...
 WHERE pph.sales_channel_id = sc.id
   ...
   AND CASE
         ...
         ELSE
          e."Дата ЭПГ"
       END >= to_date('01.10.2008', 'dd.mm.rrrr') /*«Дата платежа от» - 01.10.2008*/
