CREATE OR REPLACE FORCE VIEW V_SLIP_BACK_SCHEDULE AS
SELECT
  s.re_slip_id,
  s.re_slip_header_id,
  d.document_id,
  ...
  a.grace_date,
  a.amount,
  pkg_reins_payment.get_slip_set_off_amount(a.payment_id, 2, 1) pay_amount,
  a.contact_id,
...
WHERE  d.document_id = a.payment_id
  ...
  AND  dt.brief IN ('PayFakOutReins', 'PayFakInReins', 'PayFakInReinsOff')
