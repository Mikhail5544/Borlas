CREATE OR REPLACE FORCE VIEW V_COMMIS_FACT_PREVIEW_PREM AS
...
--       nvl(apwt.sum_premium, apdc.summ) "Базовое СГП",
--       nvl(apwt.sum_premium*apdp.policy_agent_part, apdc.summ*apdp.policy_agent_part) "Приведенное СГП",
       ...
       apd.summ*(apdp.summ/(SELECT DECODE(sum(apd3.summ),NULL,1,0,1,sum(apd3.summ))
                              FROM ag_perfom_work_dpol apd3
                             WHERE apd3.ag_perfom_work_det_id = apd.ag_perfom_work_det_id)) com_summ--"Сумма начисленого АВ"
  ...
  FROM ven_ag_roll_header arh,
       ...
       ag_perf_work_pay_doc appdp,
       ag_rate_type art,
       p_policy pp,
       p_pol_header ph,
       ven_ag_contract_header ach,
       ag_contract ac,
       ven_ag_contract_header ach_p,
       ag_contract ac_p
 WHERE 1=1
   AND arh.note IS NULL
   AND arh.ag_roll_type_id IN (62,63)
 ...
 --  and art.ag_rate_type_id = 64
 --  AND pp.pol_header_id = 11384110
  -- AND ach_p.ag_contract_header_id=569773
