create or replace view v_agn_report_npf_grs as
select 1 cntc,
     rl.ag_roll_id,
     ...
     --npf.policy_num num_pol_npf,
       ...
       case when nvl(vol.vol_amount,0) * nvl(vol.vol_rate,0) = 500 then 'Качественный'
            when nvl(vol.vol_amount,0) * nvl(vol.vol_rate,0) = 100 then 'Не качественный'
            when nvl(vol.vol_amount,0) * nvl(vol.vol_rate,0) = 250 then 'Не качественный'
            else '' end type_dog,
