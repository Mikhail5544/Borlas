CREATE OR REPLACE VIEW V_AGENT_NO_SALES_RL_OPS AS
select agent_name,
       sales_ch,
       department,
       leader_name,
...
select agent_name, sales_ch, department, leader_name, date_begin, greatest(nvl(mn.start_date,'01.01.1900'), nvl(mn.ops_start_date,'01.01.1900') ) start_date,
                      ...
                      ,case
                         when trunc(SYSDATE,'dd') - coalesce(greatest(change_date,manager_end),change_date,manager_end) < 60
                         then '< 60 дней'
                         when trunc(SYSDATE,'dd') - coalesce(greatest(change_date,manager_end),change_date,manager_end) > 60
                         then '> 60 дней'
                         when trunc(SYSDATE,'dd') - coalesce(greatest(change_date,manager_end),change_date,manager_end) = 60
                      ...
                      ,case
                        when break_ad_date > trunc(SYSDATE,'dd') then 'Да'
                        else 'Нет'
                       end as is_to_break,
                       mn.address,
                      /*,docs.change_date
                          ...
                          from ins.ven_ag_contract_header agh,
                              ...
                              ,(select agh.ag_contract_header_id,
                                ...
                                where pad.ag_contract_header_id = agh.ag_contract_header_id
                                      ...
                                      and pkg_agent_1.get_agent_new_to_policy(pad.policy_header_id,SYSDATE) = agh.ag_contract_header_id
                  ...
                  where mn.ag_contract_header_id  = docs.ag_contract_header_id (+)
                    ...
                    and nvl(br.break_ad_date,sysdate) >= SYSDATE/*to_date(
                                                         (SELECT r.param_value
                                             FROM ins_dwh.rep_param r
                                             WHERE r.rep_name = 'agentno_sell'
...
GRANT SELECT ON V_AGENT_NO_SALES_RL_OPS TO INS_EUL;