CREATE OR REPLACE FORCE VIEW V_LETTERS_PAYMENT_TIME AS
(
SELECT a.nm, a."DAYS_REMAINING",a."PAYMENT_PERIOD",a."MB",case a.mb when 0 then fee-pay_amount_dog else sum_fee-pay_amount_dog end oplata,a."POL_START_DATE",a."POL_END_DATE",a."POL_HEADER_ID",a."POLICY_ID",a."POLICY_NUMBER",a."POL_SER",a."POL_NUM",a."DOCUMENT_ID",a."NUM",a."DUE_DATE",a."GRACE_DATE",a."GRACE_PERIOD",a."AMOUNT",a."FUND",a."SUM_FEE",nvl(a.pay_amount,0) "PAY_AMOUNT",a."ACC_RATE",a."CONTACT_ID",a."CONTACT_NAME",a."INIT_NAME",a."CODE", a."ADDRESS_NAME" ,a."ID",a."DOC_STATUS_REF_ID",a."DOC_STATUS_REF_NAME",a."PRODUCT_ID",NVL2(a.pay_amount, a.amount*a.acc_rate, a.amount) rev_amount, nvl2(a.pay_amount,a.fund,'') rev_fund, a.type_contact, a.gender, a.pay_amount_dog, a.lday
FROM
(
SELECT distinct trunc(SYSDATE) - trunc(a.grace_date) days_remaining,
       row_number() over (partition by p.pol_header_id order by a.due_date desc) nm,
       CASE pt.DESCRIPTION
 ...
 WHERE a.payment_templ_id = apt.payment_templ_id
   ...
   --and p.pol_num = '002620'
   --and ph.policy_header_id = 806599
   /*AND mod(MONTHS_BETWEEN(a.DUE_DATE, p.START_DATE),12)<>0*/
   and a.due_date < to_date('15.11.2009','dd.mm.yyyy')
   ) a
)
;
