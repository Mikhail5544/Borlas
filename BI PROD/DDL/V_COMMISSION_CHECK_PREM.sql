...
SELECT --apv.ag_perf_work_vol_id,
       ...
       apw.volumes.get_volume(21) ops3_vol,
      ...
      (SELECT apd1.summ
       ...
       (CASE WHEN (tp.brief IN ('INVESTOR_LUMP','INVESTOR_LUMP_OLD') AND av.ins_period = 3 AND aca.category_name != 'Территориальный директор')
              THEN 0.3
             WHEN (tp.brief IN ('INVESTOR_LUMP','INVESTOR_LUMP_OLD') AND av.ins_period = 5 AND aca.category_name != 'Территориальный директор')
              THEN 0.2
             ELSE av.nbv_coef
       END) nbv_coef,
       /*av.nbv_coef,*/
       ...
       pkg_agn_control.get_plan_value(ach.ag_contract_header_id,5/*План ОПС*/,arh.date_end) plan_value_ops
...
SELECT --av.ag_volume_id,
 ...
 WHERE 1=1
   ...
   AND leader_ac.contract_id = leader_ach.ag_contract_header_id (+)
   AND leader_ach.agent_id = cn_leader.contact_id (+)
   AND art.brief IN ('WG_0510','SS_0510','PG_0510','OAV_GRS')
   AND apd.ag_perfom_work_det_id = apv.ag_perfom_work_det_id
   AND apv.ag_volume_id = av.ag_volume_id
   AND av.ag_contract_header_id = ach_s.ag_contract_header_id
   AND ach_s.agent_id = cn_as.contact_id
