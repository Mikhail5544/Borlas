...
--- удалять нельзя, если:
   begin
     ...
     if (v_rez = 1)then
     text_ex_error := 'Были найдены расчеты премий для агенского договора №'||p_number||',v_hid-'||v_hid;
     raise  ex_error;
  end if;
  ...
  then v_rez:=0;
   ...
   --  text_ex_error := 'Были найдены агенты для агенского договора №'||p_number||',v_hid-'||v_hid;
    -- raise  ex_error;
  end if;
