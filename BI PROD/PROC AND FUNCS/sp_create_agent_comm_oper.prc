CREATE OR REPLACE PROCEDURE Sp_Create_Agent_Comm_Oper IS
	   v_st NUMBER;
	   v_key NUMBER;
	   v_comm_amt NUMBER;
...
BEGIN
 ...
 -- Цикл по всем операциям начисления премии по договору, у которых нет агентских начислений
 FOR msfo_prem_trans_pol IN (SELECT t.*,pc.start_date,o.doc_status_ref_id FROM OPER o, TRANS t, OPER_TEMPL ot, P_COVER pc WHERE
 	 					 					 	  	 	   t.OPER_ID = o.OPER_ID
												   AND o.OPER_TEMPL_ID = ot.OPER_TEMPL_ID
												   AND ot.BRIEF= 'МСФОПремияНачисленаДог'
												   ...
												   AND o.document_ID NOT IN (SELECT oo.DOCUMENT_ID FROM OPER oo,OPER_TEMPL ott WHERE
																								  ...
																								  AND  ott.BRIEF='МСФОВознаграждениеНачисленоДог')) LOOP
		v_rez:=0;
		v_type:= NULL;
		...
		LOOP
		  ...
		  IF NVL(v_st,0)=0 THEN
		  	 dbms_output.put_line('Рассчитанное вознаграждение =0 agent_comm_id= '|| arec.p_policy_agent_com_id);
		  END IF;
          IF v_key = 0 THEN
          ...
          IF v_comm_amt <> 0 THEN
            	  ...
            	  v_res_id := Acc_New.Run_Oper_By_Template(v_msfo_comm_oper_id,
                                                     msfo_prem_trans_pol.A2_CT_URO_ID,
                                                     arec.ent_id,
