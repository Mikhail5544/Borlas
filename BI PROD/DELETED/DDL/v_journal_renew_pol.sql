CREATE OR REPLACE VIEW V_JOURNAL_RENEW_POL AS
...
from (select distinct pp.pol_header_id
      ...
      from ins.p_policy pp,
           (select min(pp.decline_date) min_date, ph.policy_header_id, pp.pol_num, ph.ids--lead(pp.policy_id) over (ORDER BY pp.decline_date)--min(pp.decline_date) md, ph.policy_header_id, pp.pol_num, ph.ids
      ...
      where gkr.policy_header_id = pp.pol_header_id
            and pp.decline_date = gkr.min_date
      ) pol,
      ins.p_pol_header ph,
      ins.p_policy p,
      ins.t_product prod,
