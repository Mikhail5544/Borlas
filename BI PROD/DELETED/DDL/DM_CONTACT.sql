...
select c.contact_id,
       c.obj_name,
       decode(ct.is_individual, 1, 'Д', 0, 'Н') is_invididual,
       cntel.tel,
       inn.id_value inn,
       pens.id_value pens,
       pass.id_value pass,
       pass_data.id_value pass_data,
       DECODE(nvl(mat_leave.contact_id,0),0,'','Декрет') mat_leave,
       p.date_of_birth,
       decode(p.gender, 1, 'М', 0, 'Ж', 'Н') gender,
       cna.country_name,
       cna.region_name,
       cna.region_type,
       cna.province_name,
       ...
       cna.address_type address_type,
        ...
        t_CONT.tel tel_CONT,
        e_mail.email,
        decode(nvl(c.resident_flag,0),1,'Резидент',0,'Не резидент','Не определно') flag_resident
...
--                       nvl(adr.name,ins.ent.obj_name(213, adr.id))*/
       ...
       (select * from
        (select ci.contact_id, ci.serial_nr || ' № ' || ci.id_value ||', Выдан: '||
                               ci.place_of_issue ||', Дата выдачи: '|| ci.issue_date id_value, row_number() over(partition by ci.contact_id order by nvl(ci.is_default,0)) rn
          from ins.cn_contact_ident ci, ins.t_id_type it
         where ci.id_type = it.id
           and it.brief = 'PASS_RF') pass1
