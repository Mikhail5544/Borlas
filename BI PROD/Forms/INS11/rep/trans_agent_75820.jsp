...
select ord, decode(ord, '1','Субагент','2','Менеджер','3','Директор агентства','') type_ag, id_num, ad_num, chn, agent_name, trans_ad_num, trans_chn, trans_agent_name, ag_stat_agent_id
from
(
(select decode(cat.category_name,'Агент','1','Менеджер','2','Директор агентства 1 года','3','Директор агентства 2 года','3','3') ord,
        decode(cat.category_name,'Агент','Субагент',cat.category_name) type_ag,
   ...
   from AG_CONTRACT_HEADER AH
        ...
        left join ag_category_agent cat on (cat.ag_category_agent_id = ac.category_id)
        left join ag_stat_agent ag_st on (ag_st.ag_stat_agent_id = pkg_agent_1.find_id_agent_status(ah.ag_contract_header_id))--Территориальный директор
        left join (select d.num trans_ad_num,
         ...
         from AG_CONTRACT_HEADER AH
              ...
              left join document d on (d.document_id = ah.ag_contract_header_id)
              left join contact c on (c.contact_id = ah.agent_id)
         where AH.AG_CONTRACT_HEADER_ID = :P_TRANS_CH_ID
              ) trans_ag on (1=1)
   where AH.AG_CONTRACT_HEADER_ID = :P_CH_ID
   		 and ag_st.name not in ('Территориальный директор','Региональный директор')
   )
  union all       
         
  (select  decode(cat.category_name,'Агент','1','Менеджер','2','Директор агентства 1 года','3','Директор агентства 2 года','3','1') ord, 
           decode(cat.category_name,'Агент','Субагент',cat.category_name),
   ...
   from AG_CONTRACT_HEADER AH
        ...
        left join ag_category_agent cat on (cat.ag_category_agent_id = lead_agl.category_id)
        left join
		(  SELECT sa.ag_stat_agent_id, sh.ag_contract_header_id, sa.name
           ...
           WHERE sh.ag_category_agent_id = ac.ag_category_agent_id
                 ...
                 AND sh.num IN (SELECT MAX(t.num) FROM ven_ag_stat_hist t WHERE t.ag_contract_header_id = sh.ag_contract_header_id)) ag_st on (ag_st.ag_contract_header_id = lead_head.ag_contract_header_id)
  ...
  ( select  decode(cat.category_name,'Агент','1','Менеджер','2','Директор агентства 1 года','3','Директор агентства 2 года','3','2') ord,
            decode(cat.category_name,'Агент','Субагент',cat.category_name),
  			    ah.ag_contract_header_id,
            dd.num,
   ...
   from AG_CONTRACT_HEADER AH
        ...
        left join ag_category_agent cat on (cat.ag_category_agent_id = dir_agl.category_id)
        left join
		(  SELECT sa.ag_stat_agent_id, sh.ag_contract_header_id, sa.name
           FROM ven_ag_stat_hist      sh,
                ven_ag_category_agent ac,
           ...
           WHERE sh.ag_category_agent_id = ac.ag_category_agent_id
                 AND sh.ag_category_agent_id = sa.ag_category_agent_id(+)
                 AND sh.ag_stat_agent_id = sa.ag_stat_agent_id(+)
                 AND sh.num IN (SELECT MAX(t.num) FROM ven_ag_stat_hist t WHERE t.ag_contract_header_id = sh.ag_contract_header_id)) ag_st on (ag_st.ag_contract_header_id = dir_head.ag_contract_header_id)
...
order by 10
	...
	<dataSource name="Q_2">
      ...
      <![CDATA[	select rownum rec_number,
	  					va.policy_header_id, 
               			va.num,
               			va.INSURER_NAME
              from v_ag_policy_current_new va where va.AG_CONTRACT_HEADER_ID = :P_CH_ID
	    ]]>
      </select>
  ...
  from v_ag_policy_current_new va 
...
<tr height=40 style='height:30.0pt'>
 ...
 <rw:foreach id="fi2" src="group_pol">
