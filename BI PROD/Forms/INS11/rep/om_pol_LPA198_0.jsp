 ...
 beforeReportTrigger="afterpform" unitOfMeasurement="centimeter">
  ...
  <data>
    ...
    <userParameter name="PA_DOC" datatype="character" width="400"
     precision="10" defaultWidth="0" defaultHeight="0"/>
	 <userParameter name="addendum_note" datatype="character" width="6000"
     precision="10" defaultWidth="0" defaultHeight="0"/>
    ...
    <userParameter name="ADMIN_PAY" datatype="character" width="200"
     precision="10" defaultWidth="0" defaultHeight="0"/>
    <userParameter name="INSURER_INN" datatype="character" width="2000"
     precision="10" defaultWidth="0" defaultHeight="0"/>
	 <userParameter name="INS_LIC" datatype="character" width="1000"
     precision="10" defaultWidth="0" defaultHeight="0"/>
    ...
    <userParameter name="PA_ROW_PRIL" datatype="number" precision="10"
     defaultWidth="0" defaultHeight="0"/>
	<userParameter name="par_group_prod" datatype="number" precision="10"
     defaultWidth="0" defaultHeight="0"/>
...
SELECT years
      ...
      ,brief
      ,dd
  FROM (SELECT ROUND(MONTHS_BETWEEN(pc.end_date, pc.start_date) / 12) years
              ,pc.start_date
...
select pkg_rep_utils.to_money_sep(pc.ins_amount) as dop_summ,
       ...
       '* "'||decode(plo.description,'Дополнительный инвестиционный доход','ИНВЕСТ',
									 ...
									 'Инвалидность Застрахованного по любой причине (1,2 группы)','ИНВАЛИДНОСТЬ ЗАСТРАХОВАННОГО ПО ЛЮБОЙ ПРИЧИНЕ (1 ГРУППЫ)',
...
(
  select
  'ДОГОВОР СТРАХОВАНИЯ ЗАКЛЮЧЕН НА ОСНОВАНИИ "ПОЛИСНЫХ УСЛОВИЙ К ДОГОВОРУ СТРАХОВАНИЯ ЖИЗНИ ПО ПРОГРАММАМ СТРАХОВАНИЯ ЖИЗНИ "ГАРМОНИЯ ЖИЗНИ", "СЕМЬЯ", "ДЕТИ", "БУДУЩЕЕ", УТВЕРЖДЕННЫХ ГЕНЕРАЛЬНЫМ ДИРЕКТОРОМ ООО "СК "РЕНЕССАНС ЖИЗНЬ" 24.06.2013 ГОДА, ДАЛЕЕ "ПОЛИСНЫЕ УСЛОВИЯ".' text,
  '*' prog
  from dual
  union all
  select
...
--  from dual
  ...
  union all
  select
  'ПРИ ДОСРОЧНОМ  РАСТОРЖЕНИИ ДОГОВОРА ВЫПЛАТА ПРЕДУСМОТРЕННОЙ СООТВЕТСТВУЮЩЕЙ ПРОГРАММОЙ СТРАХОВАНИЯ ВЫКУПНОЙ СУММЫ ПРОИЗВОДИТСЯ СОГЛАСНО ТАБЛИЦЕ ГАРАНТИРОВАННЫХ ВЫКУПНЫХ СУММ  (ПРИЛОЖЕНИЕ № 3).' text,
  '*' prog
  from dual
  ...
  union all
  select
  case :CUR_POLICY when 121 then 'РАСЧЕТ ГАРАНТИРОВАННЫХ СТРАХОВЫХ СУММ И СТРАХОВЫХ ПРЕМИЙ ПРОИЗВЕДЕН С УЧЕТОМ ГАРАНТИРОВАННОЙ НОРМЫ ДОХОДНОСТИ - 3%. ' when 122 then 'РАСЧЕТ ГАРАНТИРОВАННЫХ СТРАХОВЫХ СУММ И СТРАХОВЫХ ПРЕМИЙ ПРОИЗВЕДЕН С УЧЕТОМ ГАРАНТИРОВАННОЙ НОРМЫ ДОХОДНОСТИ - 4%. ' else 'РАСЧЕТ ГАРАНТИРОВАННЫХ СТРАХОВЫХ СУММ И СТРАХОВЫХ ПРЕМИЙ ПРОИЗВЕДЕН С УЧЕТОМ ГАРАНТИРОВАННОЙ НОРМЫ ДОХОДНОСТИ - 3%. ' end as text,
  '*' prog
  from dual
  union all
  select
  ...
  union all
  select
  'УЧИТЫВАЯ СВЕДЕНИЯ, СОДЕРЖАЩИЕСЯ В ЗАЯВЛЕНИИ НА СТРАХОВАНИЕ № '||:P_NOTICE||', ДОПОЛНИТЕЛЬНО К СОБЫТИЯМ УКАЗАННЫМ В П.4 "ПОЛИСНЫХ УСЛОВИЙ" '||upper(:addendum_note) text,
  '*' prog
  from dual
where :addendum_note <> 'X'
 ) 
 where prog like '%'||:BRIEF_PRODUCT||'%'
    or prog like '%*%']]>
      ...
      <group name="G_text">
        ...
        />
        <dataItem name="text" datatype="vchar2" columnOrder="13" width="6000"
         defaultWidth="100000" defaultHeight="10000" columnFlags="1"
         defaultLabel="Text">
          <dataDescriptor expression="rownum || text"
           descriptiveExpression="TEXT" order="1" width="6000"/>
        </dataItem>
      </group>
    </dataSource>
...
and pc.decline_date is null
 ...
 and upper(trim(plo.description)) <> 'АДМИНИСТРАТИВНЫЕ ИЗДЕРЖКИ'
 and ig.brief = 'Acc'
 and plo.id not in (147775,147819,147750,22603,32005,44179,22629,27622,27907,27624,28063,28065,28336,32005,43607,43609,44103,44131,44177,57134,57138,57738,57802,57871,57925,58264,28200,148357,148290,148315,148369,148375,148303,148308,148326,148334,148348,148353,148367,148376,148304,148309,148324,148333) -- согласовано с Д.Ивановым! добавлено 32005,28200 Веселуха
]]>
      </select>
...
begin
	v_signer := pkg_report_signer.get_signer_id_by_exe_name('ins11/om_pol_LPA198_0.jsp');
	:P_GENITIVE_POSITION_NAME := v_signer.job_position_genitive;
	:P_CASE_GENITIVE_SIGNER_NAME := pkg_contact.get_fio_fmt(pkg_contact.get_fio_case(v_signer.name,'Р'),4);
...
exception
 ...
 begin
   select v.COMPANY_TYPE || ' ' || v.COMPANY_NAME || ', ' || v.LEGAL_ADDRESS,
     ...
     ' В ' || v.BANK_COMPANY_TYPE || ' "' || v.BANK_NAME ||'", г.Москва '||decode(nvl(v.BIK,' '),' ','',', БИК ' || v.BIK) ||
     decode(nvl(v.ACCOUNT_CORR,' '),' ','',', К/С  ' || v.ACCOUNT_CORR)||',',
	 'лицензия С № 3972 77 от 17 января 2006 г., тел.: (495) 981-2-981, факс: (495) 589-18-65'
   into :INSURER,:INSURER_INN, :INS_LIC
   from v_company_info v;
  exception
      when no_data_found then :INSURER := ''; :INSURER_INN := ''; :INS_LIC := '';
  end;
  
begin
SELECT decode(sc.brief
             ,'RLA'
             ,'Ренессанс Лайф Актив'
             ,nvl(ent.obj_name(con_a.ent_id, con_a.contact_id), ' ')) ag_fio
      ,decode(sc.brief, 'RLA', '45082', nvl(ah.NUM, ' ')) tn
  INTO :PA_CURATOR
      ,:PA_TABNUM
 ...
 WHERE pp.pol_header_id = ph.policy_header_id
   ...
   AND sc.id = ah.t_sales_channel_id
   AND rownum = 1;
exception
...
decode(vf.brief, 
           ...
           'EUR', 'ЕВРО',
           vf.brief) kod_val_up,
	to_char(round(extract(year from pp.end_date + interval '1' second)-extract(year from ph.start_date),2))||' лет  с "'||to_char(ph.start_date,'dd')||'" '||case extract(month from ph.start_date) when 1 then 'января' 
                                                                 when 2 then 'февраля' 
                                                                 when 3 then 'марта'
                                                                 when 4 then 'апреля'
                                                                 when 5 then 'мая'
	...
	CASE WHEN ak.brief IN ('END','CHI','PEPR','TERM','END_2','CHI_2','PEPR_2','TERM_2')
               THEN 1
         ELSE 0
         END
    into :BRIEF_PRODUCT,:policy_number,:POL_NUM,:P_NOTICE,:CUR_POLICY,:PROGRAMMA,:P_SYSDATE,:TYPE_CONT,:POL_HOLDER,:PH_BIRTH,:PH_DOC,:POL_ASS,:PA_BIRTH,:PA_DOC, :PA_PERIODICH, :PA_DAYS,:PA_PER, :WAITING_PERIOD_END_DATE, :PA_ROW_PRIL, :PA_KOD_VAL,:PA_KOD_VAL_UP, :Y, :seria, :par_group_prod
...
-- Дата уплаты первого последующих взносов
	...
	-- Байтин А.
	-- Исключить аннулированные
	-- Веселуха
	-- Оставила Артура исключение аннулирование, но минимальный после аннулированного
    -- Чирков --339837
    -- Переделал с payment_number на функцию, возвращающую минимальный payment_id для минимальной due_date.
	and ap.payment_id = pkg_payment.get_min_epg_payment_id(pp.policy_id);
  exception
...
for rec in (select decode(rownum,1,'дополнительные программы:' ||t.description,', ' || t.description) val  
           from (select '"' || decode(plo.description,'Дополнительный инвестиционный доход','Инвест',
            ...
            where pp.policy_id = :P_POLICY_ID
             ...
             and tl.t_lob_id = tll.t_lob_id
             and ig.t_insurance_group_id = tll.insurance_group_id
             and (ig.brief <> 'Acc' or plo.id in (147775,147819,147750,22603,32005,44179,22629,27622,27907,27624,28063,28065,28336,32005,43607,43609,44103,44131,44177,57134,57138,57738,57802,57871,57925,58264,28200,148357,148290,148315,148369,148375,148303,148308,148326,148334,148348,148353,148367,148376,148304,148309,148324,148333)) -- согласовано с Д.Ивановым! добавлено 32005,28200 Веселуха
union
...
"Ренессанс Жизнь", именуемое в дальнейшем "Страховщик", <rw:field id="" src="P_GENITIVE_POSITION_NAME"/><o:p></o:p> <rw:field id="" src="P_CASE_GENITIVE_SIGNER_NAME"/><o:p></o:p>
, действующего на основании Доверенности № <rw:field id="" src="P_PROCURORY_NUM"/><o:p></o:p>, с одной стороны, <o:p></o:p></span></p>
...
<table class=MsoNormalTable border=0 cellspacing=0 cellpadding=0 width=708
 ...
 <tr style='mso-yfti-irow:4'>
  <td width=708 colspan=5 valign=top style='width:531.0pt'>
 ...
 <tr style='mso-yfti-irow:4'>
  ...
  <p class=MsoNormal><span style='font-size:7.0pt;mso-bidi-font-size:8.0pt;
  mso-bidi-font-size:8.0pt;mso-ansi-language:RU'>
  <rw:field id="" src="INS_LIC"/></span></p>
  </td>
 </tr>
...
<table class=MsoNormalTable border=0 cellspacing=0 cellpadding=0 width=708
 ...
 <tr style='mso-yfti-irow:23;height:4.0pt'>
  ...
  RU'>Гарантированная страховая сумма,<br><rw:field id="" src="PA_KOD_VAL"/></span></i></p>
  </td>
  <td width=110 valign=top style='width:82.2pt;padding:0cm 5.4pt 0cm 5.4pt;
  height:4.0pt;border:dashed windowtext 1.0pt;'>
  <p class=MsoNormal align=center style='text-align:center;'><i
...
<tr style='mso-yfti-irow:8'>
 ...
 для соответствующей валюты на дату уплаты. Все банковские расходы, связанные с оплатой страховой
  премии (взноса), оплачиваются Страхователем. <% } else { %> Все банковские расходы, связанные с оплатой страховой
  премии (взноса), оплачиваются Страхователем. <% } %></span></i><b style='mso-bidi-font-weight:
...
%> 
 ...
 <tr style='mso-yfti-irow:34'>
  ...
  страхования жизни по программам страхования жизни "Гармония жизни", "Семья", "Дети", "Будущее" от 24.06.2013 г.</span></i></p>
  </td>
 </tr>
 ...
 <tr style='mso-yfti-irow:36'>
  ...
  <td width=444 colspan=9 valign=top style='width:333.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal><i style='mso-bidi-font-style:normal'><span
  style='font-size:8.0pt;mso-ansi-language:RU'><% if (!jProdBrief.equals("TERM") ) { %><%=format.format(NN++)%>. Приложение №3: Таблица гарантированных выкупных сумм по
...
<rw:getValue id="t_group_prod" src="par_group_prod"/> 
 <tr style='mso-yfti-irow:38'>
  <td width=264 colspan=3 valign=top style='width:198.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal><span style='font-size:8.0pt;mso-ansi-language:RU'><o:p>&nbsp;</o:p></span></p>
  </td>
 ...
 <% if (t_flg.equals("0")) { %> 2. Во всем остальном, что не 
  предусмотрено настоящим Дополнительным Соглашением Стороны руководствуются положениями Полисных условий к 
  договору страхования жизни <% } else { %> 2. В соответствие с изменениями, указанными в п. 1
 ...
 <tr style='mso-yfti-irow:38'>
  ...
  <td width=444 colspan=9 valign=top style='width:333.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal align=right><span style='font-size:7.0pt;mso-ansi-language:RU'>
  3. Во всем остальном, что не предусмотрено настоящим Дополнительным Соглашением Стороны руководствуются 
 ...
 <tr style='mso-yfti-irow:38'>
  ...
  <td width=444 colspan=9 valign=top style='width:333.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal align=right><span style='font-size:7.0pt;mso-ansi-language:RU'>
  <% if (t_flg.equals("0")) { %>3.<% } else { %>4.<% } %>
  <% if (t_group_prod.equals("0")) { %>Настоящее Дополнительное соглашение 
  вступает в силу с момента оплаты дополнительного страхового взноса в сумме и в сроки, указанные в п. 2 настоящего Дополнительного соглашения.<% }
  else { %>Настоящее Дополнительное соглашение вступает в силу с момента оплаты  страхового взноса в полном объеме в сумме и в сроки, указанные в п. 4 настоящего Дополнительного соглашения.<% } %>
  </span></p>
  </td>
 </tr>
  </tr>
  ...
  <td width=444 colspan=9 valign=top style='width:333.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal align=right><span style='font-size:7.0pt;mso-ansi-language:RU'>
  <% if (t_flg.equals("0")) { %>4.<% } else { %>5.<% } %> Настоящее Дополнительное соглашение является 
  составной и неотъемлемой частью Полиса страхования жизни № <rw:field id="" src="SERIA"/> <rw:field id="" src="P_NOTICE"/></span></p>
  </td>
 </tr>
 <tr style='mso-yfti-irow:38'>
  <td width=264 colspan=3 valign=top style='width:198.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
...
<table class=MsoNormalTable border=0 cellspacing=0 cellpadding=0 width=626
 ...
 <tr style='mso-yfti-irow:0;mso-yfti-firstrow:yes;mso-yfti-lastrow:yes;
  ...
  <p class=MsoNormal align=center style='margin-right:4.55pt;text-align:center'><span
  style='mso-bookmark:table_sum'><b style='mso-bidi-font-weight:normal'><span
  style='mso-ansi-language:RU'>ТАБЛИЦА ГАРАНТИРОВАННЫХ ВЫКУПНЫХ СУММ ПО<span
