...
where pp.policy_id = :P_POLICY_ID --782299
 ...
 and upper(trim(plo.description)) <> 'АДМИНИСТРАТИВНЫЕ ИЗДЕРЖКИ'
 and (ig.brief <> 'Acc'
  or (ig.brief = 'Acc' and (plo.id = 22603 or plo.id = 32005))) -- согласовано с Д.Ивановым! добавлено 32005 Веселуха
]]>
      </select>
...
where pp.policy_id = :P_POLICY_ID --782299
 ...
 and upper(trim(plo.description)) <> 'АДМИНИСТРАТИВНЫЕ ИЗДЕРЖКИ'
 and ig.brief = 'Acc'
 and plo.id not in (22603,32005) -- согласовано с Д.Ивановым! добавлено 32005 Веселуха
]]>
      </select>
...
--     ven_p_pol_header ph
--     left join fund fl on (fl.fund_id = ph.fund_id)
--where pp.policy_id = :P_POLICY_ID
...
exception
  ...
  select  
    ...
    decode(vf.brief, 'RUR', 'руб.', vf.brief) kod_val
    into :BRIEF_PRODUCT,:POL_NUM,:P_NOTICE,:CUR_POLICY,:PROGRAMMA,:P_SYSDATE,:POL_HOLDER,:PH_BIRTH,:PH_DOC,:POL_ASS,:PA_BIRTH,:PA_DOC, :PA_PERIODICH, :WAITING_PERIOD_END_DATE, :PA_ROW_PRIL, :PA_KOD_VAL
...
<table class=MsoNormalTable border=0 cellspacing=0 cellpadding=0 width=708
 ...
 <tr style='mso-yfti-irow:42'>
  ...
  style='mso-spacerun:yes'> </span><% if (!j_has_dop.equals("0")) { %>И ДОПОЛНИТЕЛЬНЫЕ ПРОГРАММЫ<% } else { %>ПРОГРАММА<% } %> </a> <i
...
%> 
 ...
 <tr style='mso-yfti-irow:37'>
  ...
  <td width=444 colspan=9 valign=top style='width:333.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal><i style='mso-bidi-font-style:normal'><span
  style='font-size:8.0pt;mso-ansi-language:RU'><%=format.format(NN++)%>. Соглашение о временном
 ...
 <tr style='mso-yfti-irow:38'>
  ...
  <td width=444 colspan=9 valign=top style='width:333.0pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal><i style='mso-bidi-font-style:normal'><span
  style='font-size:8.0pt;mso-ansi-language:RU'><%=format.format(NN++)%>. Правила страхования жизни от 30.06.2007</span></i></p>
  </td>
 </tr>
