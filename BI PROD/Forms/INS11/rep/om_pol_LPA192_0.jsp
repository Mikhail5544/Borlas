...
(
  select
  'ДОГОВОР СТРАХОВАНИЯ ЗАКЛЮЧЕН НА ОСНОВАНИИ "ПОЛИСНЫХ УСЛОВИЙ К ДОГОВОРУ СТРАХОВАНИЯ ЖИЗНИ ПО ПРОГРАММАМ СТРАХОВАНИЯ ЖИЗНИ "ГАРМОНИЯ ЖИЗНИ", "СЕМЬЯ", "ДЕТИ", "БУДУЩЕЕ", УТВЕРЖДЕННЫХ ГЕНЕРАЛЬНЫМ ДИРЕКТОРОМ ООО "СК "РЕНЕССАНС ЖИЗНЬ" 09.11.2009 ГОДА, ДАЛЕЕ "ПОЛИСНЫЕ УСЛОВИЯ".' text,
  '*' prog
  from dual
  union all
  select
...
--  from dual
  ...
  union all
  select
  'ПРИ ДОСРОЧНОМ  РАСТОРЖЕНИИ ДОГОВОРА ВЫПЛАТА ПРЕДУСМОТРЕННОЙ СООТВЕТСТВУЮЩЕЙ ПРОГРАММОЙ СТРАХОВАНИЯ ВЫКУПНОЙ СУММЫ ПРОИЗВОДИТСЯ СОГЛАСНО ТАБЛИЦЕ ВЫКУПНЫХ СУММ  (ПРИЛОЖЕНИЕ № 3).' text,
  '*' prog
  from dual
...
begin
	...
	v_signer := pkg_report_signer.get_signer_id_by_exe_name('ins11/om_pol_LPA192_0.jsp');
...
begin
 SELECT CASE 
  ...
  WHERE pp.pol_header_id=ph.policy_header_id 
    ...
    AND ach2.ag_contract_header_id=10146042  --ООО "Ренессанс Лайф Актив"
...
%> 
 ...
 <tr style='mso-yfti-irow:34'>
  ...
  страхования жизни по программам страхования жизни "Гармония жизни", "Семья", "Дети", "Будущее" от 09.11.2009 г.</span></i></p>
  </td>
 </tr>
