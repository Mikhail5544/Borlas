...
FROM(    
    ...
    SELECT
			...
			,get_payed_cover_sum payment_amount
            --страховая сумма с учетом выплат
            ,case when one_is_aggregate = 1 
                  then ins_amount - get_payed_cover_sum/*ot_claim*/
                  else ins_amount
             end ins_paym
            --количесво дней     
            ...
            ,sum(
                case when nvl(ins_amount,0) = 0 then 0
                         ...
                         case when  (d_status_brief in ('NEW', 'CURRENT') AND 
                                      (peril_name NOT LIKE '%оспитализация%'
                                       AND UPPER(peril_name) NOT LIKE '%ВРЕМЕННАЯ НЕТРУДОСПОСОБНОСТЬ ЗАСТРАХОВАННОГО В РЕЗУЛЬТАТЕ%'
									   AND UPPER(peril_name) NOT LIKE '%ВРЕМЕННАЯ НЕТРУДОСПОСОБНОСТЬ ЗАСТРАХОВАННОГО ЛИЦА В РЕЗУЛЬТАТЕ%'
                                       AND UPPER(peril_name) NOT LIKE 'ВРЕМЕННАЯ УТРАТА ТРУДОСПОСОБНОСТИ ЗАСТРАХОВАННЫМ%')
                                     ) then
                           /**/
                           (
                            CASE WHEN a.IS_AGGREGATE = 1 THEN
                              round(a.payment_sum * a.rate / (a.ins_amount - pkg_claim.get_payed_cover_sum(a.c_claim_id, a.p_cover_id)) * 100, 1)
                            ELSE
                              round(a.payment_sum * a.rate/ a.ins_amount * 100, 1)
                            END
                            )
                           /**/
                         else 
            ...
            , sum(case when peril_name like '%оспитализация%'
                           or UPPER(peril_name) like '%ВРЕМЕННАЯ НЕТРУДОСПОСОБНОСТЬ ЗАСТРАХОВАННОГО В РЕЗУЛЬТАТЕ%'
						   or UPPER(peril_name) like '%ВРЕМЕННАЯ НЕТРУДОСПОСОБНОСТЬ ЗАСТРАХОВАННОГО ЛИЦА В РЕЗУЛЬТАТЕ%'
                           or UPPER(peril_name) like 'ВРЕМЕННАЯ УТРАТА ТРУДОСПОСОБНОСТИ ЗАСТРАХОВАННЫМ%'
                     then   
					 		(select decode(pct.brief,
                                  'DAMAGE_PERC',
                                  dc.limit_val,
                                  'DAMAGE_DAYS',
                                  0.2,
                                  null)
                          from ins.ven_t_damage_code dc, ins.ven_p_cover pc, ins.ven_t_prod_coef_type pct
                         where a.t_damage_code_id = dc.id
                           and a.p_cover_id = pc.p_cover_id
                           and pct.t_prod_coef_type_id = dc.t_prod_coef_type_id) 
        ...
        FROM
            ...
            when plo.description like 'Первичное диагностирование смертельно опасного заболевания'
                 ...
                 ,d.c_claim_id
                 ,d.p_cover_id
				 ,pkg_claim.get_payed_cover_sum(d.c_claim_id, d.p_cover_id) get_payed_cover_sum
				 ,d.t_damage_code_id
      ...
      select
        ...
        ,  (select to_char(max(pc_1.end_date),'DD.MM.YYYY') 
         ...
         , CASE WHEN (SELECT COUNT(*)
                      FROM ins.ven_c_event ea,
                           ins.c_claim_header ccha,
                           ins.t_peril tper,
                           ins.c_claim cca,
                           ins.document d
                      WHERE ea.c_event_id = e.c_event_id
                            AND ea.c_event_id = ccha.c_event_id 
                            AND ccha.peril_id = tper.id
                            AND ccha.active_claim_id = cca.c_claim_id
                            AND cca.c_claim_id = d.document_id
                            AND d.doc_status_ref_id = (SELECT rf.doc_status_ref_id
                                                       FROM ins.doc_status_ref rf
                                                       WHERE rf.brief = 'DECISION')
                            AND (UPPER(tper.description) like UPPER('Смерть застрахованного%')
                                 OR UPPER(tper.description) like UPPER('Смерть Страхователя%')
                                )
                      ) > 0
                  THEN pkg_utils.get_aggregated_string(cast(multiset(
...
<p class=MsoNormal align=center style='text-align:center'><span
style='font-size:6.0pt'>Ф.И.О. Начальника отдела финансового мониторинга<o:p></o:p></span></p>
