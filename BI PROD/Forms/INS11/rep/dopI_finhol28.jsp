  ...
  where pp.policy_id = :POL_ID
   ...
   AND pl.t_lob_line_id NOT IN (SELECT lb.t_lob_line_id
	...
	<dataSource name="Q_RESERVE_PAY">
      <select>
      <![CDATA[select pkg_rep_utils.to_money(pc.premium) RESERVE_PAY
    from 
            ins.p_policy pp, 
            ins.as_asset ass, 
            ins.p_cover pc, 
...
where pp.policy_id = :POL_ID;
exception
      when no_data_found then :pol_header := 0; 
  end;
...
style='font-size:14.0pt'>Дополнительное соглашение № <%=VERSION_NUM%></span></p>

<p class=MsoNormal align=center style='text-align:center'><span
style='font-size:12.0pt'>к Договору страхования № <%=pol_ser_num%> от <%=CONFIRM_DATE%></span></p>
...
<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. Изложить раздел 4 
...
<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0
 ...
 <tr style='mso-yfti-irow:1'>
  ...
  mso-border-alt:dashed windowtext .5pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal align=center style='text-align:center'><i
  style='mso-bidi-font-style:normal'><span style='font-size:8.0pt'>Страховой взнос, <rw:field id="" src="PA_KOD_VAL"/><o:p></o:p></span></i></p>
  </td>
 </tr>
...
<% if (!curr.equals("руб.")) { %> Рублевый эквивалент премии уплачивается по курсу ЦБ РФ, установленному
 для соответствующей валюты на дату уплаты. Все банковские расходы, связанные с оплатой страхового
  взноса, оплачиваются Страхователем<% } else { %> Все банковские расходы, связанные с оплатой страхового
  взноса, оплачиваются Страхователем<% } %>.<o:p></o:p></span></i></p>
  </td>
 </tr>
...
<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. Изложить раздел 5
Договора в следующей редакции:</p>
...
<p class=MsoNormal style='text-align:justify'><b style='mso-bidi-font-weight:
normal'><i style='mso-bidi-font-style:normal'><span style='font-size:9.0pt'>При
 условии своевременной оплаты Страхователем очередного страхового взноса, указанного
в п. 1 настоящего Дополнительного соглашения, срок страхования по указанной дополнительной 
программе будет считаться пролонгированным на 1 год на прежних условиях соответствующей 
...
<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. Изложить пункт 5 раздела 
8 "Дополнительные условия" в следующей редакции: "При досрочном расторжении (прекращении) Договора 
Страховщик не выплачивает выкупные суммы".</p>

<p class=MsoNormal style='text-align:justify'><o:p>&nbsp;</o:p></p>

<rw:getValue id="z" src="amount_z"/>
<rw:getValue id="inv" src="ex_invest"/>
<rw:getValue id="ex_inv_res" src="ex_invest_reserve"/>

<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. 
<% if (inv.equals("0")) {%> <%} else {%> В связи с расторжением Договора в отношении программы "Инвест" исключить 
Приложение № 2 к Договору. Страхователю причитается выплата в размере выкупной суммы, <rw:field id="" src="s_surr_reserve"/> <rw:field id="" src="PA_KOD_VAL"/> <%} %> 
<% if (ex_inv_res.equals("0")) {%> <%} else {%> В связи с расторжением Договора в отношении программы "Инвест-Резерв" исключить 
Приложение № 1 б к Договору. Страхователю причитается выплата в размере выкупной суммы, <rw:field id="" src="s_surr_reserve"/> <rw:field id="" src="PA_KOD_VAL"/> <%} %> 
<% if (z.equals("1")) { %>Исключить Приложение № 1 к Договору.<% } else { %> Исключить Приложение № 1 к Договору. Страхователь обязан 
оплатить задолженность перед Страховщиком в размере просроченных страховых взносов, <rw:field id="" src="amount_z"/> <rw:field id="" src="PA_KOD_VAL"/>
<% } %> Выкупную сумму зачесть в счет оплаты страховых взносов. </p>

<p class=MsoNormal style='text-align:justify'><o:p>&nbsp;</o:p></p>

<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. Изменения и дополнения к 
Договору, указанные в п. 1-4 настоящего Дополнительного соглашения, действуют с 
<%=CONFIRM_DATE_ADDENDUM%>.</p>

<p class=MsoNormal style='text-align:justify'><o:p>&nbsp;</o:p></p>

<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. Настоящее Дополнительное 
соглашение вступает в силу с момента подписания его Сторонами.</p>

<p class=MsoNormal style='text-align:justify'><o:p>&nbsp;</o:p></p>

<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. Настоящее Дополнительное 
соглашение является составной и неотъемлемой частью Договора страхования № <%=pol_ser_num%> от <%=CONFIRM_DATE%>.</p>

<p class=MsoNormal style='text-align:justify'><o:p>&nbsp;</o:p></p>

<p class=MsoNormal><span style='letter-spacing:-.1pt'><%=format.format(NUM_PP++)%>. Настоящее Дополнительное 
Соглашение подписано в двух экземплярах, имеющих одинаковую силу, по одному экземпляру для каждой Стороны.</span></p>
