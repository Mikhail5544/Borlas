...
(select start_date,
        NUM,
        pol,
        case when s_z >= (amount) then 1 else 0 end flag,
        case when s_z >= (amount) then s_z else s_v end amount,
		brief,
        fund_id,
        bank,
        bank_adr,
		...
		code
 from (
   SELECT to_char(ph.start_date,'dd.mm.yyyy') start_date,
       e.NUM,
       decode(p.pol_ser, null, p.pol_num, p.pol_ser || '-' || p.pol_num) pol,
       sum(nvl(pkg_renlife_utils.ret_amount_active(ch.c_claim_header_id,'PAYORDER_SETOFF'),0)) s_z,
       sum(nvl(pkg_renlife_utils.ret_amount_active(ch.c_claim_header_id,'PAYORDER'),0)) s_v,
