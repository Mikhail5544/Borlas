...
(
  select
  'ДОГОВОР СТРАХОВАНИЯ ЗАКЛЮЧЕН НА ОСНОВАНИИ "ПОЛИСНЫХ УСЛОВИЙ К ДОГОВОРУ СТРАХОВАНИЯ ЖИЗНИ ПО ПРОГРАММАМ СТРАХОВАНИЯ ЖИЗНИ "ГАРМОНИЯ ЖИЗНИ", "СЕМЬЯ", "ДЕТИ", "БУДУЩЕЕ", УТВЕРЖДЕННЫХ ГЕНЕРАЛЬНЫМ ДИРЕКТОРОМ ООО "СК "РЕНЕССАНС ЖИЗНЬ" 27.04.2011 ГОДА, ДАЛЕЕ "ПОЛИСНЫЕ УСЛОВИЯ".' text,
  '*' prog
  from dual
  union all
  select
...
begin
	...
	v_signer := pkg_report_signer.get_signer_id_by_exe_name('ins11/om_pol_LPA195_0.jsp');
...
begin
 SELECT CASE 
        WHEN dep.name='Агентство RLA' THEN 
         nvl(cn2.obj_name_orig,' ') 
        ELSE nvl(ent.obj_name(con_a.ent_id,con_a.contact_id),' ')
         END ag_fio,
        CASE 
        WHEN dep.name='Агентство RLA' THEN 
         nvl(ach2.num,' ') 
   ...
   FROM ins.ven_p_pol_header ph 
       ...
       ,ins.ven_ag_contract_header ah 
       ,ins.ven_contact con_a
       ,ins.ven_employee ve
       ,ins.ag_contract  ac
       ,ins.department   dep
  ...
  WHERE pp.pol_header_id=ph.policy_header_id 
    ...
    AND ach2.ag_contract_header_id=80462244  --ООО "Ренессанс Лайф Актив"
    AND cn2.contact_id    = ach2.agent_id
  ...
  ( select document_id, start_date from (
    ...
    where ds.doc_status_ref_id = dsr.doc_status_ref_id(+)
      and dsr.brief = 'ACTIVE' and ds.document_id = :P_POLICY_ID order by ds.start_date desc ) where rownum = 1) ds,
         V_POL_LPA_REPORT comp /*17.04.2014 Черных М. по заявке 324573 (исправлено задвоение адреса страхователя)*/
    where pp.policy_id = :P_POLICY_ID
...
"Ренессанс Жизнь", именуемое в дальнейшем "Страховщик", в лице <rw:field id="" src="P_GENITIVE_POSITION_NAME"/><o:p></o:p> <rw:field id="" src="P_CASE_GENITIVE_SIGNER_NAME"/><o:p></o:p>
...
%> 
 ...
 <tr style='mso-yfti-irow:34'>
  ...
  страхования жизни по программам страхования жизни "Гармония жизни", "Семья", "Дети", "Будущее" от 27.04.2011 г.</span></i></p>
  </td>
 </tr>
