...
--  from dual
  ...
  union all
  select
  'ПРИ ДОСРОЧНОМ  РАСТОРЖЕНИИ ДОГОВОРА ВЫПЛАТА ПРЕДУСМОТРЕННОЙ СООТВЕТСТВУЮЩЕЙ ПРОГРАММОЙ СТРАХОВАНИЯ ВЫКУПНОЙ СУММЫ ПРОИЗВОДИТСЯ СОГЛАСНО ТАБЛИЦЕ ГАРАНТИРОВАННЫХ ВЫКУПНЫХ СУММ  (ПРИЛОЖЕНИЕ № 1).' text,
  '*' prog
  from dual
 ...
 where prog like '%'||:BRIEF_PRODUCT||'%'
    or prog like '%*%']]>
      ...
      <group name="G_text">
        ...
        />
        <dataItem name="text" datatype="vchar2" columnOrder="13" width="2000"
         defaultWidth="100000" defaultHeight="10000" columnFlags="1"
         defaultLabel="Text">
          <dataDescriptor expression="rownum || text"
           descriptiveExpression="TEXT" order="1" width="2000"/>
        </dataItem>
      </group>
    </dataSource>
    <dataSource name="Q_uslov_add">
      ...
      <![CDATA[
select
  CASE WHEN :BRIEF_PRODUCT not in 'TERM' THEN 
  						(CASE WHEN :CUR_POLICY <> 122 THEN 
							(CASE WHEN :PRINT_8_4 = '1' THEN '11. ' else '10. ' end)
						ELSE (CASE WHEN :PRINT_8_4 = '1' THEN '10. ' else '9. ' end)
						 END)
  	ELSE (CASE WHEN :CUR_POLICY <> 122 THEN 
			(CASE WHEN :PRINT_8_4 = '1' THEN '10. ' else '9. ' end)
		  ELSE (CASE WHEN :PRINT_8_4 = '1' THEN '9. ' else '8. ' end)
		  END)
...
-- Дата уплаты первого последующих взносов
    ...
    decode(nvl(pt.number_of_payments,0),0,'X',1,to_char(pps.due_date,'dd.mm'),
                      12,to_char(add_months(pps.due_date,1),'dd.mm') || ', ' ||to_char(add_months(pps.due_date,2),'dd.mm') || ', ' ||
                                        to_char(add_months(pps.due_date,3),'dd.mm')||' '||'числа каждого месяца года срока страхования',                                              
...
@page Section1
    ...
    margin:70.9pt 1.27cm 45.1pt 1.27cm;
...
@page Section2
    {size:595.3pt 841.9pt;
    margin:70.9pt 1.27cm 152.8pt 1.27cm;
    mso-header-margin:35.45pt;
    mso-footer-margin:14.2pt;
    mso-paper-source:0;}
div.Section2
...
<table class=MsoNormalTable border=0 cellspacing=0 cellpadding=0 width=708
 ...
 <tr style='mso-yfti-irow:27'>
  <td width=455 colspan=2 valign=top style='width:341.3pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal><b style='mso-bidi-font-weight:normal'><span
  style='font-size:8.0pt;mso-ansi-language:RU'>ДОПОЛНИТЕЛЬНЫЕ ПРОГРАММЫ:<o:p></o:p><b>*</b></span></b></p>
  </td>
  <td width=143 colspan=2 valign=top style='width:107.5pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal align=center style='text-align:center'><b
  style='mso-bidi-font-weight:normal'><span style='font-size:8.0pt;mso-ansi-language:
...
<tr style='mso-yfti-irow:8'>
  ...
  <td width=110 valign=top style='width:82.0pt;border-top:dashed windowtext 1.0pt;border-left:
  dashed windowtext 1.0pt;border-bottom:dashed windowtext 1.0pt;border-right:dashed windowtext 1.0pt;
  mso-border-top-alt:dashed windowtext .5pt;mso-border-left-alt:dashed windowtext .5pt;
  mso-border-alt:dashed windowtext .5pt;padding:0cm 5.4pt 0cm 5.4pt'>
  <p class=MsoNormal align=center style='text-align:center'><b
...
<br></br>
 ...
 <tr style='mso-yfti-irow:23;page-break-inside:avoid;height:1.0pt'>
  ...
  height:1.9pt'>
  <p class=MsoNormal style='margin-left:5.4pt;text-indent:-5.4pt'><span
  style='font-size:6.0pt;mso-ansi-language:RU'><o:p>&nbsp;</o:p></span></p>
  </td>
 </tr>
 </rw:foreach>
