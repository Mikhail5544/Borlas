...
<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>. Изложить раздел <% if (t_check.equals("1")) { %>6<% } else { %>5<% } %>
Договора в следующей редакции:</p>
...
При условии своевременной оплаты Страхователем очередного страхового взноса, указанного
...
<p class=MsoNormal style='text-align:justify'><%=format.format(NUM_PP++)%>.<% if (ex_inv.equals("0")) {%> <%} else {%> В связи с
расторжением Договора в отношении программы "Инвест" исключить Приложение № 2 к Договору. Страхователю причитается
выплата в размере выкупной суммы, <rw:field id="" src="s_surr"/> <rw:field id="" src="PA_KOD_VAL"/> <%} %> 
<% if (ex_inv_res.equals("0")) {%> <%} else {%> В связи с расторжением Договора в отношении программы "Инвест-Резерв" исключить 
Приложение № 1 а к Договору. Страхователю причитается выплата в размере выкупной суммы, <rw:field id="" src="s_surr_reserve"/> <rw:field id="" src="PA_KOD_VAL"/> <%} %> 
<% if (z.equals("1")) {%>Исключить Приложение № <% if (t_check.equals("1")) { %>3<% } else { %>1<% } %> к Договору.<%} else {%> Исключить Приложение № <% if (t_check.equals("1")) { %>3<% } else { %>1<% } %> к Договору. Страхователь обязан оплатить задолженность перед Страховщиком в размере просроченных страховых взносов, 
