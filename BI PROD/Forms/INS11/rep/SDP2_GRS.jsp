  ...
  <data>
    ...
    <dataSource name="Q_AGENT_REPORT">
      ...
      <![CDATA[SELECT count(1) over() row_cnt, h.ag_contract_header_id,
       ...
       decode(ch.description,'ГРС Москва, Санкт-Петербург','GRS-MOS','ГРС Регионы','GRS-REG',ch.description) sales_ch,
	 ...
	 case ch.description when 'DSF' then 'Приказ №15' 
	 					 when 'SAS' then 'Приказ №13'
						 when 'SAS 2' then 'Приказ №14'
						 when 'ГРС Москва, Санкт-Петербург' then 'Приказ №34-2'
						 when 'ГРС Регионы' then 'Приказ №34-1'
						 else '' end prikaz
 ...
 WHERE 1=1
   ...
   and apw.ag_perfomed_work_act_id = :p_ag_perfomed_work_act_id
    ...
    <![CDATA[select 1 ret, to_char(nvl((select sum(nvl(pf.vol_amount,0) * nvl(pf.vol_rate,0) + (nvl(vol.vol_amount,0) * nvl(vol.vol_rate,0)))
     ...
     join ins.ag_rate_type rt on det.ag_rate_type_id = rt.ag_rate_type_id and rt.brief = 'OAV_GRS'
...
from ins.ag_roll_header rh 
     ...
     left join ins.ag_perfomed_work_act act on act.ag_roll_id = rl.ag_roll_id
     left join ins.ag_perfom_work_det det on det.ag_perfomed_work_act_id = act.ag_perfomed_work_act_id
	 left join ins.ag_rate_type rt on (det.ag_rate_type_id = rt.ag_rate_type_id and rt.brief = 'OAV_GRS')
     left join ins.ag_perf_work_vol vol on vol.ag_perfom_work_det_id = det.ag_perfom_work_det_id
     left join ins.ag_volume av on av.ag_volume_id = vol.ag_volume_id
     left join ins.ag_npf_volume_det npf on npf.ag_volume_id = av.ag_volume_id
...
mso-bidi-font-size:10.0pt;font-family:"Times New Roman"'>Приложение №1 к СУБАГЕНТСКОМУ ДОГОВОРУ (ПРИСОЕДИНЕНИЯ) категории <rw:field id="" src="sales_ch"/> <o:p></o:p></span></p>
...
1. Настоящее Приложение № 1 является неотъемлемой частью Субагентского договора (ПРИСОЕДИНЕНИЯ) категории <rw:field id="" src="sales_ch"/> 
в редакции от 26 июня 2011 г. утвержденного Генеральным директором ЗАО "РенЛайфПартнерс" Киселевым О.М.
от 24 июня 2011 г. <rw:field id="" src="prikaz"/> (Далее - СДП).<o:p></o:p></span></p>
<p class=MsoTitle style='margin-top:6.0pt;text-align:justify;tab-stops:0in'><span
lang=RU style='font-size:11.0pt;mso-bidi-font-size:10.0pt;font-family:"Times New Roman"'>
2. Настоящее Приложение № 1 устанавливает форму Отчета АГЕНТА к Субагентскому договору (ПРИСОЕДИНЕНИЯ) категории <rw:field id="" src="sales_ch"/>,
...
<p class=MsoTitle style='margin-top:6.0pt;text-align:justify;tab-stops:0in'><span
lang=RU style='font-size:11.0pt;mso-bidi-font-size:10.0pt;font-family:"Times New Roman"'>
Закрытое акционерное общество "РенЛайфПартнерс", именуемое в дальнейшем АГЕНТ, в лице 
...
<table class=MsoNormalTable border=0 cellspacing=0 cellpadding=0 width="100%"
 ...
 <tr style='mso-yfti-irow:4;height:21.2pt'>
  ...
  height:21.2pt'>
  <p class=MsoNormal><b><span style='font-size:8.0pt;font-family:Arial;
  mso-bidi-font-family:"Times New Roman"'>Расчет субагентского вознаграждения по Договорам ОПС, заключенным между НПФ
