  ...
  <data>
    ...
    <dataSource name="Q_AGENT">
      ...
      <![CDATA[SELECT ach.agent_id, ach.ag_contract_header_id,
       ...
       nvl(dept_exe.contact_id,1) dir_contact_id,
       ...
       NVL (case when nvl( (select genitive from contact where contact_id = dept_exe.contact_id),
                 ent.obj_name(ent.id_by_brief ('CONTACT'), dept_exe.contact_id)) = 'Глушковой Натальи Николаевны' then
                 'Директора Департамента продаж через агентскую сеть Глушковой Натальи Николаевны' 
                 else '_______________________________________' end,
            'Директора Департамента продаж через агентскую сеть Глушковой Натальи Николаевны'
           ) dir_name,
...
<rw:getValue id="DIR_CONTACT_ID" src="DIR_CONTACT_ID"/>
...
СТРАХОВЩИК, в лице </span><span style='font-size:11.0pt;font-family:"Times New Roman";
mso-ansi-language:EN-US'><%=CHIEF_NAME%></span><span
lang=RU style='font-size:11.0pt;font-family:"Times New Roman"'>, действующей(его)
на основании Доверенности № <% if (DIR_CONTACT_ID.equals("257976")) { %><%=PNUM_DOC_DIR%><% } else { %>______________________<% } %>
, выданной <% if (DIR_CONTACT_ID.equals("257976")) { %><%=DATAV_DOC_DIR%><% } else { %>______________________<% } %>, с одной
...
«СТРАХОВЩИК», в лице </span><span style='font-size:11.0pt;mso-ansi-language:
EN-US'><%=CHIEF_NAME%></span><span lang=RU
style='font-size:11.0pt;layout-grid-mode:line'>, действующей(его) на основании </span><span
lang=RU style='font-size:11.0pt'>Доверенности № 
<% if (DIR_CONTACT_ID.equals("257976")) { %><%=PNUM_DOC_DIR%><% } else { %>______________________<% } %>
, выданной <% if (DIR_CONTACT_ID.equals("257976")) { %><%=DATAV_DOC_DIR%><% } else { %>______________________<% } %>
...
дальнейшем «АГЕНТ», действующим на основании Свидетельства о государственной регистрации 
физического лица в качестве индивидуального предпринимателя серия <%=POBUL_SER%> № </span></span><span lang=RU><%=POBUL_NUM%></span><span
...
EN-US;layout-grid-mode:line'> Обществом с ограниченной ответственностью "Страховая Компания "Ренессанс Жизнь"</span><span lang=RU style='font-size:11.0pt'>
